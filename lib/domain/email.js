...
/* ------------------------------------------------------------------ *
 * storageKey — a filename that Supabase Storage will actually accept
 * ------------------------------------------------------------------ */

/**
 * `safeFilename` makes a name safe to SHOW and to write to a disk. It is not
 * enough for an object key, and the difference took production down.
 *
 * Supabase Storage accepts roughly S3's safe set. `[`, `]`, `#`, `{`, `}`,
 * `^`, backtick and `~` are all outside it and all survive `safeFilename`,
 * which strips only the characters a filesystem objects to.
 *
 * ⚠️ WHY THIS IS NOT A RARE EDGE CASE. Microsoft 365 and Google Workspace
 * prepend `[EXTERNAL]` to the subject of mail from outside the organisation,
 * and vendors tag their own — `[ServeManager]`. So almost every message the
 * firm receives from an adjuster, a defence firm or a process server carries
 * brackets in its subject, and the subject becomes the `.eml` filename. Every
 * one of them was rejected with a 400.
 *
 * `#` is worse than merely illegal: it is a URL fragment delimiter, so the
 * key silently TRUNCATES at the client. `Fwd: [ServeManager] Job #4171.eml`
 * reached Supabase as `Fwd_ [ServeManager] Job ` — no extension, trailing
 * space, and nothing in the request to say a character had been eaten.
 *
 * Kept SEPARATE from `safeFilename` on purpose. A displayed attachment name
 * should keep the brackets and the accents somebody actually typed; only the
 * key needs to be timid. The two are stored side by side — `name` for people,
 * `path` for the bucket.
 *
 * Existing rows are unaffected: `signAttachment` reads the `path` that was
 * stored with the row rather than recomputing it, so keys written under the
 * old scheme keep resolving.
 */
export function storageKey(name, fallback = 'message') {
  const clean = (part) =>
    String(part || '')
      // Mendez, not M_ndez: fold accents rather than blanking them.
      .normalize('NFKD')
      .replace(/[\u0300-\u036f]/g, '')
  ...
  /*
   * An extension, or a sentence? "Garcia vs. Jackson" ends in a dot-word too.
   * A short run with no space in it is an extension; anything else is prose
   * and stays part of the name.
   */
  const isExt = dot > 0 && raw.length - dot <= 6 && !/\s/.test(raw.slice(dot));
  const ext = isExt ? clean(raw.slice(dot + 1)) : '';
  const suffix = ext ? `.${ext}` : '';

  // Trim the BASE to fit, so the extension survives. Truncating the whole
  // string is how `...assigned.eml` became `...assigne`.
  const base = clean(isExt ? raw.slice(0, dot) : raw).slice(0, 120 - suffix.length);
  // The slice can re-expose a trailing space that `clean` had removed.
  const trimmed = base.replace(/[\s._\-]+$/, '');

  return trimmed ? `${trimmed}${suffix}` : `${fallback}${suffix}`;
}

/**
 * Where attachment `index` of a message lives, relative to the message folder.
 *
 * ⚠️ THE INDEX IS WHAT MAKES THIS SAFE. Keys used to be the filename alone,
 * and uploads use `upsert: true` so re-filing a message overwrites in place.
 * Together those meant one message with two attachments called `image.png` --
 * every Outlook signature does this -- wrote both to the same key, and the
 * second silently replaced the first. Both cards then opened the same bytes.
 * Names that merely CLEAN to the same key (`#1.pdf` and `_1.pdf`) collided
 * the same way.
 *
 * A folder per attachment, not a prefix on the name: the object keeps its own
 * filename, which is what a browser offers when somebody saves it, and
 * `1/...` can never equal the original's `<subject>.eml` beside it.
 *
 * Still deterministic -- parse order is stable -- so re-filing the same
 * message overwrites its own files and nobody else's. Rows written under the
 * old scheme keep working because `signAttachment` reads the stored `path`.
 */
export function attachmentKey(index, filename) {
  return `${index + 1}/${storageKey(filename, 'attachment')}`;
}
