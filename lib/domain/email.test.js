...
/* ------------------------------------------------------------------ *
 * storageKey — the keys Supabase Storage rejected
 * ------------------------------------------------------------------ */

/** Supabase Storage / S3 reject these outright. */
const ILLEGAL_IN_KEY = /[[\]#{}^~`\\|<>%]/;

test('the three keys that actually 400ed in production are now legal', () => {
  /*
   * Verbatim from the Supabase storage log, 2026-09-04 04:32. Every one was a
   * 400, every 400 became a 500, and every 500 was retried once a minute for
   * four days. The bracket is inserted by Microsoft 365 and Google Workspace,
   * not by anybody at the firm, so this was most of the inbound mail.
   */
  const subjects = [
    'Fwd: [ServeManager] Job #4171 assigned',
    'Re: [EXTERNAL] Garcia vs. Jackson-0805304730.1',
    'Re: [EXTERNAL MAIL] Higdon Lawyers Law Firm - Robertson, Linda MMD',
  ];
  for (const subject of subjects) {
    const key = storageKey(`${safeFilename(subject) || 'message'}.eml`);
    assert.ok(!ILLEGAL_IN_KEY.test(key), `${key} still holds an illegal character`);
    assert.ok(key.endsWith('.eml'), `${key} lost its extension`);
    assert.equal(key, key.trim(), `${key} has surrounding whitespace`);
  }
});

test('a # does not silently truncate the key at the URL fragment', () => {
  // The worst of the three: `#` is a fragment delimiter, so Supabase received
  // `Fwd_ [ServeManager] Job ` and the extension never arrived at all.
  const key = storageKey('Fwd_ [ServeManager] Job #4171 assigned.eml');
  assert.ok(!key.includes('#'));
  assert.ok(key.endsWith('.eml'));
  assert.ok(/4171/.test(key), 'the part after the # must survive, not be dropped');
});

test('the extension survives a long subject', () => {
  // safeFilename caps at 120, and appending ".eml" then capping AGAIN at 120
  // is what chopped the extension off. The base is trimmed instead.
  const key = storageKey(`${'x'.repeat(200)}.eml`);
  assert.ok(key.endsWith('.eml'), key.slice(-10));
  assert.ok(key.length <= 120, `length ${key.length}`);
});

test('a name that sanitizes away falls back rather than ending in a slash', () => {
  // An inline signature image often has no filename. An empty last segment
  // makes the object key end in "/", which is a 400 of its own.
  for (const v of ['', '   ', '###', '[[[]]]', null, undefined]) {
    const key = storageKey(v);
    assert.ok(key.length > 0, `${JSON.stringify(v)} produced an empty key`);
    assert.ok(!ILLEGAL_IN_KEY.test(key), key);
  }
  assert.equal(storageKey('', 'message'), 'message');
});

test('a dotted phrase is not mistaken for an extension', () => {
  // "Garcia vs. Jackson" ends in a dot-word; chopping at the last dot would
  // treat " Jackson" as the extension and rebuild the name wrongly.
  const key = storageKey('Garcia vs. Jackson');
  assert.ok(key.includes('Jackson'), key);
  assert.ok(!key.endsWith('.Jackson'), key);
});

test('accents fold instead of blanking out a client name', () => {
  assert.equal(storageKey('Méndez récords.pdf'), 'Mendez records.pdf');
});

test('safeFilename is deliberately left alone, because people read it', () => {
  // The display name keeps what somebody typed; only the key is timid. If this
  // ever starts stripping brackets, the UI has quietly lost information.
  assert.equal(safeFilename('Re: [EXTERNAL] hello'), 'Re_ [EXTERNAL] hello');
});

/* ------------------------------------------------------------------ *
 * attachmentKey — two attachments, two objects
 * ------------------------------------------------------------------ */

test('two attachments with the same name in one email get different keys', () => {
  // Outlook signatures: every inline logo is image.png. Keyed by name alone,
  // the second upsert overwrote the first and both cards opened one file.
  const names = ['image.png', 'image.png', 'image001.jpg', 'image001.jpg'];
  const keys = names.map((n, i) => attachmentKey(i, n));
  assert.equal(new Set(keys).size, keys.length, keys.join(', '));
});

test('names that only clean to the same key still get different keys', () => {
  // `#` and `_` both become `_`, so storageKey alone cannot tell these apart.
  assert.equal(storageKey('#1.pdf'), storageKey('_1.pdf'));
  assert.notEqual(attachmentKey(0, '#1.pdf'), attachmentKey(1, '_1.pdf'));
});

test('the filename itself is kept, in its own folder', () => {
  // What a browser offers when somebody saves the file.
  assert.equal(attachmentKey(0, 'Outlook-Please con.png'), '1/Outlook-Please con.png');
  assert.equal(attachmentKey(2, 'Records [final].pdf'), '3/Records _final.pdf');
  assert.ok(attachmentKey(0, '').startsWith('1/'), 'a nameless attachment still gets a key');
});

test('an attachment key can never land on the original .eml', () => {
  // The original sits beside the attachments as `<subject>.eml`, no folder.
  const original = storageKey('image.eml');
  assert.notEqual(attachmentKey(0, 'image.eml'), original);
  assert.ok(attachmentKey(0, 'image.eml').includes('/'));
});

test('the same message produces the same keys every time', () => {
  // Re-filing overwrites in place -- its own files, and only its own.
  assert.equal(attachmentKey(4, 'image.png'), attachmentKey(4, 'image.png'));
});
