...
/* ------------------------------------------------------------------ *
 * Who a generated deadline lands on
 * ------------------------------------------------------------------ */

const solMatter = (attorney) => ({
  m1: { values: { sol: '2027-06-01', attorney } },
});

test('a deadline goes to the case attorney', () => {
  const tasks = generateChainTasks(solMatter('Rose Hassanzai'));
  const sol = Object.values(tasks).find((t) => t.ruleKey === 'sol');
  assert.equal(sol.assignedTo, 'Rose Hassanzai');
});

test('a task already sitting on Unassigned picks the attorney up later', () => {
  /*
   * ⚠️ "Unassigned" IS A TRUTHY STRING, and the old code read
   * `existing?.assignedTo || values.attorney`. So every deadline generated
   * before an attorney was filled in kept "Unassigned" permanently -- the
   * firm set the attorney and nothing moved.
   */
  const before = generateChainTasks(solMatter(''));
  const sol = Object.values(before).find((t) => t.ruleKey === 'sol');
  assert.equal(sol.assignedTo, 'Unassigned', 'no attorney yet');

  const after = generateChainTasks(solMatter('Rose Hassanzai'), before);
  const moved = Object.values(after).find((t) => t.ruleKey === 'sol');
  assert.equal(moved.assignedTo, 'Rose Hassanzai', 'it stayed on nobody');
});

test('a task somebody deliberately assigned is NOT reassigned', () => {
  // Changing the attorney on a case must not sweep work off the desk of
  // whoever was actually given it.
  const before = generateChainTasks(solMatter('Rose Hassanzai'));
  const id = Object.keys(before).find((k) => before[k].ruleKey === 'sol');
  before[id] = { ...before[id], assignedTo: 'monica' };

  const after = generateChainTasks(solMatter('John Paul Bográn Jr.'), before);
  assert.equal(after[id].assignedTo, 'monica');
});

test('"unassigned" in any casing counts as nobody', () => {
  const before = generateChainTasks(solMatter(''));
  const id = Object.keys(before).find((k) => before[k].ruleKey === 'sol');
  for (const spelling of ['unassigned', 'UNASSIGNED', '  Unassigned  ', '']) {
    const seeded = { ...before, [id]: { ...before[id], assignedTo: spelling } };
    const after = generateChainTasks(solMatter('Rose Hassanzai'), seeded);
    assert.equal(after[id].assignedTo, 'Rose Hassanzai', `spelling ${JSON.stringify(spelling)}`);
  }
});
