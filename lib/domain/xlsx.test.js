...
test('A CHECKBOX IN A DATE COLUMN IS NOT A DATE', async () => {
  /*
   * The bug that reached production. "Mediation Done?" is formatted as a date
   * and holds 0 or 1. Converted blindly that is 1899-12-30 -- a real-looking
   * date that nothing downstream questions, which then failed the
   * occurred_on_range check in Postgres. The import reported success and those
   * checklist rows silently did not land.
   */
  const styles = '<styleSheet><cellXfs count="1"><xf numFmtId="14"/></cellXfs></styleSheet>';
  const [row] = await readXlsx(book(
    '<row r="1"><c r="A1" s="0"><v>1</v></c><c r="B1" s="0"><v>0</v></c>'
    + '<c r="C1" s="0"><v>46251</v></c></row>', { styles }));
  assert.equal(row[0], '1', 'kept as the number it is, not blanked and not 1899-12-31');
  assert.equal(row[1], '0');
  assert.equal(row[2], '2026-08-17', 'a real date still converts');
});

test('serialToISO refuses a serial outside a case file\'s lifetime', () => {
  assert.equal(serialToISO(1), '');
  assert.equal(serialToISO(0), '');
  assert.equal(serialToISO(19999), '', 'just before 1954');
  assert.equal(serialToISO(60001), '', 'just after 2064');
  assert.equal(serialToISO(20000), '1954-10-03');
  assert.equal(serialToISO(60000), '2064-04-08');
});
...
test('serials before March 1900 are off by one, and that is accepted', () => {
   ...
   */
  // Now moot in practice: everything below serial 20000 is refused outright,
  // so the phantom-leap-day discrepancy cannot reach a case file at all. Kept
  // as a statement of the epoch this uses.
  assert.equal(serialToISO(20000), '1954-10-03');
});
