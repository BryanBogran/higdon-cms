...
import {
  ...
  applyReduction, buildLines, centsToInput,
...
/* ------------------------------------------------------------------ *
 * Per-line dollar overrides
 * ------------------------------------------------------------------ */

const meds = [
  { id: 'r1', provider: 'Post Oak Chiropractic', amount: '6000' },
  { id: 'r2', provider: 'I-10 MRI', amount: '16500' },
];

test('an override replaces the billed amount without changing the row', () => {
  /*
   * The firm's ask: "change the values in the settlement calculator without
   * affecting the bills". The bill is what a provider charged; the settlement
   * figure is what was negotiated. Writing the second over the first loses
   * the first.
   */
  const r = computeSettlement({
    gross: '100000', medicalRows: meds, overrides: { r1: '4200' },
  });
  const line = r.medicals.lines.find((l) => l.id === 'r1');
  assert.equal(line.billedCents, 600000, 'the bill itself is untouched');
  assert.equal(line.originalCents, 420000, 'the calculator uses the override');
  assert.equal(line.reducedCents, 420000);
  // The other line is unaffected.
  assert.equal(r.medicals.lines.find((l) => l.id === 'r2').originalCents, 1650000);
});

test('an override and a reduction compose: 6,000 -> 5,000 -> 4,500', () => {
  // The documented order. Both controls keep the meaning they already had.
  const r = computeSettlement({
    gross: '100000', medicalRows: meds, overrides: { r1: '5000' }, reductions: { r1: 10 },
  });
  const line = r.medicals.lines.find((l) => l.id === 'r1');
  assert.equal(line.billedCents, 600000);
  assert.equal(line.originalCents, 500000);
  assert.equal(line.reducedCents, 450000);
});

test('an unreadable or cleared override falls back to the bill, not to zero', () => {
  /*
   * A half-typed override must not silently zero a medical bill and inflate
   * what the client appears to be owed.
   */
  for (const bad of ['', '   ', 'abc', null, undefined]) {
    const r = computeSettlement({
      gross: '100000', medicalRows: meds, overrides: { r1: bad },
    });
    assert.equal(
      r.medicals.lines.find((l) => l.id === 'r1').originalCents,
      600000,
      `override ${JSON.stringify(bad)} should fall back to the bill`,
    );
  }
});

test('an override of zero is honoured — a bill negotiated away is not missing', () => {
  // 0 and null are different answers here, as they are for liens.
  const r = computeSettlement({
    gross: '100000', medicalRows: meds, overrides: { r1: '0' },
  });
  const line = r.medicals.lines.find((l) => l.id === 'r1');
  assert.equal(line.originalCents, 0);
  assert.equal(r.medicals.missing, 0, 'zero is a value, not an absence');
});

test('overrides change what the client nets, and the columns still reconcile', () => {
  const base = computeSettlement({ gross: '100000', medicalRows: meds });
  const cut = computeSettlement({ gross: '100000', medicalRows: meds, overrides: { r1: '1000' } });
  assert.ok(cut.netToClientCents > base.netToClientCents, 'a smaller bill leaves the client more');
  assert.equal(
    cut.grossCents - cut.feeCents - cut.medicalCents - cut.expenseCents - cut.lienCents - cut.otherCents,
    cut.netToClientCents,
  );
});

test('showing the lien total does not change what the client nets', () => {
  /*
   * THE ASSERTION THAT STOPS A CLIENT BEING UNDERPAID.
   *
   * The firm asked for lien amounts to "show up in the settlement
   * calculator". Displaying is not deducting: on a PI file the provider bills
   * usually ARE the liens, so subtracting both takes the same money off the
   * client twice. `totalLiens` is what the UI displays; `lienCents` is what is
   * subtracted, and with the toggle off that must stay zero.
   */
  const liens = [{ id: 'l1', amount: '3000' }, { id: 'l2', amount: '1200', reduction: '200' }];

  const shown = totalLiens(liens);
  assert.equal(shown.cents, 400000, '3,000 + (1,200 - 200) = 4,000');

  const off = computeSettlement({ gross: '100000', lienRows: liens, includeLiens: false });
  assert.equal(off.lienCents, 0, 'nothing is deducted while the toggle is off');

  const on = computeSettlement({ gross: '100000', lienRows: liens, includeLiens: true });
  assert.equal(on.lienCents, 400000);
  assert.ok(on.netToClientCents < off.netToClientCents, 'deducting leaves the client less');
});

/* ------------------------------------------------------------------ *
 * Typing money — reported 2026-09-23
 *
 * "When I try to input a decimal and then cents on the 'use' button in
 * the settlement calc, it does not let me."
 *
 * The override box rendered the PARSED value back into the input on every
 * keystroke, so the decimal point was erased on the stroke that made it.
 * These pin the two halves of the fix: what the box must hold while a
 * number is half-typed, and what that half-typed string means.
 * ------------------------------------------------------------------ */

/** What a controlled input ends up holding, keystroke by keystroke. */
function typeInto(text, render) {
  let shown = '';
  for (const ch of text) shown = render(shown + ch);
  return shown;
}

test('⚠️ re-rendering the parsed value makes cents impossible to type', () => {
  /*
   * THE BUG, kept executable. This is the old policy: show
   * String(parseMoney(typed) / 100). Typing 1234.56 lands on $123,456.00 --
   * a hundred times the intended figure, on the sheet a client is read
   * their money off.
   */
  const reparsed = (typed) => {
    const cents = parseMoney(typed);
    return cents === null ? '' : String(cents / 100);
  };
  assert.equal(typeInto('1234.56', reparsed), '123456');
  assert.equal(parseMoney(typeInto('1234.56', reparsed)), 12345600);
});

test('showing what was typed lets the whole number be typed', () => {
  // The fix: the box holds the string, untouched. parseMoney reads it.
  const asTyped = (typed) => typed;
  assert.equal(typeInto('1234.56', asTyped), '1234.56');
  assert.equal(parseMoney(typeInto('1234.56', asTyped)), 123456);
});

test('a half-typed amount reads as what is there so far, not as zero', () => {
  // The total moves while you type and settles when you stop. It must not
  // read a trailing dot as nothing and zero out a provider mid-keystroke.
  assert.equal(parseMoney('1234'), 123400);
  assert.equal(parseMoney('1234.'), 123400);
  assert.equal(parseMoney('1234.5'), 123450);
});

test('an empty box is the one that falls back to the billed amount', () => {
  // null, not 0 -- "no override" and "override of nothing" are different.
  assert.equal(parseMoney(''), null);
  const lines = buildLines(
    [{ id: 'r1', provider: 'Memorial Hermann', amount: '6000' }],
    { labelKeys: ['provider'], amountKey: 'amount', overrides: { r1: '' } },
  ).lines;
  assert.equal(lines[0].originalCents, 600000, 'a cleared override must not become $0');
});

test('the override survives the trip and the reduction still applies to it', () => {
  // 6,000 billed -> 5,000 used -> less 10% = 4,500, typed as "5000.00".
  const { lines } = buildLines(
    [{ id: 'r1', provider: 'Memorial Hermann', amount: '6000' }],
    { labelKeys: ['provider'], amountKey: 'amount',
      overrides: { r1: '5000.00' }, reductions: { r1: '10' } },
  );
  assert.equal(lines[0].billedCents, 600000);
  assert.equal(lines[0].originalCents, 500000);
  assert.equal(lines[0].reducedCents, 450000);
});

test('a reduction of 33.33% can be typed too', () => {
  // Same fault, same table: value={l.percent || 0} stalled "33.33" at "33."
  const { lines } = buildLines(
    [{ id: 'r1', provider: 'X', amount: '1000' }],
    { labelKeys: ['provider'], amountKey: 'amount', reductions: { r1: '33.33' } },
  );
  assert.equal(lines[0].percent, 33.33);
  assert.equal(lines[0].reducedCents, 66670);
});

test('centsToInput keeps the trailing zero a placeholder needs', () => {
  assert.equal(centsToInput(123450), '1234.50');
  assert.equal(centsToInput(123456), '1234.56');
  assert.equal(centsToInput(0), '0.00');
  assert.equal(centsToInput(-500), '-5.00');
