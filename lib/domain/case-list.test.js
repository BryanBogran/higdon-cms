...
import {
  ...
  sortRows, matchesCaseType, matchesDepo, buildCaseList, directionLabel, CHECKLIST_FILTERS, matchesChecklist, toggleChecklistFilter,
...
test('each deposition filter selects what it says', () => {
  ...
  /*
   * 'both' replaced 'either' at the firm's request: the two single-party
   * options already cover "either" between them, and the question worth asking
   * is which cases have BOTH sides done and can move on.
   */
  assert.deepEqual(
    [plOnly, defOnly, both, none].map((x) => matchesDepo(x, 'both')),
    [false, false, true, false],
  );
  // The retired key must not quietly behave like something else.
  assert.deepEqual(
    [plOnly, defOnly, both, none].map((x) => matchesDepo(x, 'either')),
    [true, true, true, true],
    'an unknown filter matches everything rather than emptying the list',
  );
  assert.deepEqual(
...
test('every checklist item has a done and a not-done option', () => {
  /*
   * Derived from FIELDS rather than counted by hand, so adding a checklist
   * item cannot leave it unfilterable -- which is the whole point. Counter
   * Affidavits Filed and Damaged Witness Deposition arrived this way and
   * needed no change here.
   */
  const items = FIELDS.filter((f) => f.type === 'yesnoDoc');
  assert.equal(CHECKLIST_FILTERS.length, items.length * 2);
  assert.ok(items.length >= 15, `expected at least 15 checklist items, found ${items.length}`);
...
/* ------------------------------------------------------------------ *
 * Several checklist conditions at once
 * ------------------------------------------------------------------ */

test('many conditions mean ALL of them, not any', () => {
  /*
   * The query that makes this a work queue: suit filed, no answer yet. OR
   * would answer a question nobody asked -- "cases where either of two
   * unrelated things happened".
   */
  const awaitingAnswer = { values: { suitFiled: { done: true }, answerFiled: { done: false } } };
  const answered = { values: { suitFiled: { done: true }, answerFiled: { done: true } } };
  const notFiled = { values: {} };

  const f = ['suitFiled:done', 'answerFiled:open'];
  assert.equal(matchesChecklist(awaitingAnswer, f), true);
  assert.equal(matchesChecklist(answered, f), false, 'answered is not awaiting an answer');
  assert.equal(matchesChecklist(notFiled, f), false, 'no suit filed at all');
});

test('a single string still works, so nothing that already called this breaks', () => {
  const m = { values: { affidavitsFiled: { done: true } } };
  assert.equal(matchesChecklist(m, 'affidavitsFiled:done'), true);
  assert.equal(matchesChecklist(m, 'affidavitsFiled:open'), false);
});

test('an empty selection matches everything', () => {
  const m = { values: {} };
  for (const f of [[], '', null, undefined, [null, '']]) {
    assert.equal(matchesChecklist(m, f), true, JSON.stringify(f));
  }
});

test('toggling the opposite state of the same item replaces it', () => {
  /*
   * Under AND, "Suit Filed — done" plus "Suit Filed — not done" can never
   * match anything. A filter that silently guarantees zero results is worse
   * than one that refuses the contradiction, so the newer pick wins.
   */
  const after = toggleChecklistFilter(['suitFiled:done'], 'suitFiled:open');
  assert.deepEqual(after, ['suitFiled:open']);
});

test('toggling the same option twice turns it off', () => {
  // The option is its own undo, so a mis-click needs no trip to the chip row.
  const on = toggleChecklistFilter([], 'mediation:done');
  assert.deepEqual(on, ['mediation:done']);
  assert.deepEqual(toggleChecklistFilter(on, 'mediation:done'), []);
});

test('different items accumulate', () => {
  let sel = [];
  for (const f of ['suitFiled:done', 'answerFiled:open', 'mediation:done']) {
    sel = toggleChecklistFilter(sel, f);
  }
  assert.deepEqual(sel, ['suitFiled:done', 'answerFiled:open', 'mediation:done']);
});

test('buildCaseList narrows with each condition added', () => {
  const matters = {
    m1: { values: { clientName: 'A', caseNumber: '26-001', suitFiled: { done: true }, answerFiled: { done: false } } },
    m2: { values: { clientName: 'B', caseNumber: '26-002', suitFiled: { done: true }, answerFiled: { done: true } } },
    m3: { values: { clientName: 'C', caseNumber: '26-003' } },
  };
  assert.equal(buildCaseList(matters, { checklist: [] }).length, 3);
  assert.equal(buildCaseList(matters, { checklist: ['suitFiled:done'] }).length, 2);
  assert.deepEqual(
    buildCaseList(matters, { checklist: ['suitFiled:done', 'answerFiled:open'] }).map((r) => r.id),
    ['m1'],
  );
});
