...
/* ------------------------------------------------------------------ *
 * fmtLong — for documents
 * ------------------------------------------------------------------ */

test('fmtLong spells the month out', () => {
  assert.equal(fmtLong('2026-09-23'), 'September 23, 2026');
  assert.equal(fmtLong('2023-01-12'), 'January 12, 2023');
  assert.equal(fmtLong('1998-03-04'), 'March 4, 1998');
});

test('fmtLong does not shift the day in any timezone', () => {
  /*
   * The reason the suite runs three times. A date-only value rendered through
   * the viewer's zone lands a day early or late either side of midnight, and
   * a date of injury off by one is a statute of limitations off by one.
   */
  assert.equal(fmtLong('2026-01-01'), 'January 1, 2026');
  assert.equal(fmtLong('2026-12-31'), 'December 31, 2026');
});

test('⚠️ fmtLong leaves a blank, where fmt would print an em dash', () => {
  // An em dash on the "Date of Birth:" line of a letter to a provider reads
  // as though the firm withheld it. A blank reads as an oversight.
  assert.equal(fmtLong(''), '');
  assert.equal(fmtLong(null), '');
  assert.equal(fmtLong('not a date'), '');
  assert.equal(fmt(''), '—', 'fmt keeps its own behaviour for tables');
});

test('fmtLong takes an explicit fallback when a caller wants one', () => {
  assert.equal(fmtLong('', { fallback: 'unknown' }), 'unknown');
});
