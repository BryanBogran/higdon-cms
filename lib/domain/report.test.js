...
test('BUG_REPORT_TO can name several people', () => {
  /*
   * A comma-separated To is valid RFC 5322 and Gmail honours every address in
   * it, so routing reports to more than one person needs no code change --
   * only the env var. Asserted rather than assumed, because assertHeaderSafe
   * sits between the value and the header and could plausibly have been
   * written to reject anything that is not a single address.
   */
  const msg = buildMessage({
    to: 'paul@higdonlawyers.com, bryan@higdonlawyers.com',
    from: 'files@higdonlawyers.com',
    subject: 's',
    text: 't',
  });
  assert.match(msg, /^To: paul@higdonlawyers\.com, bryan@higdonlawyers\.com$/m);
});

test('a line break still cannot hide inside a recipient list', () => {
  assert.throws(
    () => buildMessage({
      to: 'paul@higdonlawyers.com,\r\nBcc: exfil@attacker.tld',
      from: 'f@x.com', subject: 's', text: 't',
    }),
    /Recipient contains a line break/,
  );
});

test('the new-report email links to the ticket when given one', () => {
  const report = validateReport({ summary: 'SOL not saving' }).report;
  const body = reportEmailBody(report, { displayName: 'Monica' }, { link: 'https://x.test/?help=abc' });
  assert.match(body, /Open the request: https:\/\/x\.test\/\?help=abc/);
});

test('without a link the email is unchanged', () => {
  const report = validateReport({ summary: 'SOL not saving' }).report;
  assert.doesNotMatch(reportEmailBody(report, { displayName: 'Monica' }), /Open the request/);
});
