...
export function planCreateFromFolders(folders = [], matters = {}) {
  ...
  const live = Object.entries(matters).filter(([, m]) => !m?.archivedAt);
  const linkedFolderIds = new Set(live.map(([, m]) => m?.driveFolderId).filter(Boolean));
  const numbersInUse = new Set(
    live.map(([, m]) => String(m?.values?.caseNumber || '').trim()).filter(Boolean)
  );
  ...
  for (const folder of folders) {
    ...
    if (linkedFolderIds.has(folder?.id)) {
      ...
      skipped.push({ folder, reason: `case ${caseNumber} already exists — the sync links it` });
    } else if (counts.get(caseNumber) > 1) {
      skipped.push({
        folder,
        reason: `${counts.get(caseNumber)} folders carry ${caseNumber} — pick which is the case`,
      });
    } else {
      create.push({
        folder,
...
export function planFolderCreate(folderName, matters = {}) {
  ...
  const holder = caseNumber
    ? Object.entries(matters).find(
      ([, m]) => String(m?.values?.caseNumber || '').trim() === caseNumber && !m?.archivedAt,
    )
    : undefined;
