 ...
 */
/*
 * The range is narrow ON PURPOSE, and this is the bug it exists to stop.
 *
 * A "Mediation Done?" column formatted as a date but holding 0 or 1 -- a
 * checkbox, in other words -- converts to 1899-12-30. That is a real date, so
 * nothing downstream questions it, and it reached Postgres as a checklist
 * completion date and was refused by `occurred_on_range`. The import looked
 * like it worked and those rows silently did not land.
 *
 * 20000-60000 is 1954 to 2064, matching the identical guard already in
 * lib/domain/import.js for bare integers in CSV. A case file has no date
 * outside that, and a number outside it is not a date -- it is a flag, a
 * count, or a stray.
 */
const SERIAL_MIN = 20000; // 1954-10-03
const SERIAL_MAX = 60000; // 2064-04-08

export function serialToISO(serial) {
  const n = Number(serial);
  if (!Number.isFinite(n) || n < SERIAL_MIN || n > SERIAL_MAX) return '';
  ...
  for (const rowMatch of xml.matchAll(/<row\b[^>]*?(?:\/>|>([\s\S]*?)<\/row>)/g)) {
    ...
    for (const c of inner.matchAll(/<c\b([^>]*?)(?:\/>|>([\s\S]*?)<\/c>)/g)) {
      ...
      if (type === 's') {
        ...
        /*
         * `|| raw` matters. A date-styled cell holding something that is not a
         * date keeps its original value rather than being blanked, so it shows
         * up on the import review screen as the number it actually is and a
         * person can see why that column did not map.
         */
        value = raw && dateStyles.has(style) ? (serialToISO(raw) || raw) : raw;
      }
