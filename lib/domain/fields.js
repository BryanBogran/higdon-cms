...
export const FIELDS = [
  ...
  /*
   * 'person', not 'text'. Free text gave the firm TWELVE spellings of two
   * attorneys -- "John Paul Bogran, Jr.", "John Paul Bográn Jr", "JP", and
   * two with a leading space. None of them matched a profile, so the
   * deadline chain could not assign a single task and everything the system
   * generated sat Unassigned.
   *
   * The options come from the staff roster at runtime, so they cannot be
   * listed here. FieldInput resolves them; a stored value that matches
   * nobody is still shown rather than silently blanked.
   */
  { key: 'attorney', label: 'Attorney', type: 'person', section: 'Case Info' },
  {
    key: 'status',
    label: 'Status',
    type: 'select',
