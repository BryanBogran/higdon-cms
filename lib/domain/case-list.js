...
export const DEPO_FILTERS = [
  ...
  // 'both' replaced 'either' at the firm's request. "Either" answered a
  // question nobody had -- the two single-party options already cover it
  // between them, and what a paralegal wants is the cases where BOTH sides are
  // done and the case can move on.
  { key: 'both', label: 'Both depos taken' },
...
export function matchesChecklist(matter, filter) {
  /*
   * Accepts one filter or many, and MANY MEANS ALL OF THEM.
   *
   * AND, not OR, because that is what makes this a work queue: "Suit Filed
   * done" plus "Answer Filed not done" is the list of cases waiting on an
   * answer, which is a question somebody actually has. OR would answer
   * "cases where either of two unrelated things happened", which is not.
   *
   * It also matches how every other facet on the page already combines --
   * status AND attorney AND case type. One control behaving differently from
   * its neighbours is a trap.
   *
   * A single string still works, so existing callers and bookmarked URLs are
   * unaffected.
   */
  ...
  return filters.every((f) => {
    const [key, state] = String(f).split(':');
    // An unparseable entry matches rather than empties the list -- a stale
    // bookmark should not look like "no cases".
...
/**
 * Add a filter to the selection, keeping one state per checklist item.
 *
 * Picking "Suit Filed — not done" when "Suit Filed — done" is already on
 * REPLACES it rather than adding it. Under AND the two together can never
 * match anything, and a filter that silently guarantees zero results is worse
 * than one that refuses the contradiction.
 */
export function toggleChecklistFilter(selected = [], filter) {
  if (!filter) return selected;
  const key = String(filter).split(':')[0];

  // Off if already on, so the same option is its own undo.
  if (selected.includes(filter)) return selected.filter((f) => f !== filter);

  return [...selected.filter((f) => String(f).split(':')[0] !== key), filter];
}
