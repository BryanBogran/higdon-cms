 ...
 */
export function reportEmailBody(report, reporter, { when = '', link = '' } = {}) {
  const lines = [
    report.summary,
    '',
  ];
  ...
  /*
   * The way into the ticket. Without it the email is the whole conversation
   * and the status never moves -- the thing the ticket system exists to fix.
   */
  if (link) {
    lines.push('', '───────────────', `Open the request: ${link}`,
      'Set its status and reply in the app, so the reporter can see it.');
  }
