...
/* ------------------------------------------------------------------ *
 * The date of accident, under Filevine's spellings
 * ------------------------------------------------------------------ */

test('Filevine spellings of the date of accident all map to doa', () => {
  /*
   * 0 of the 344 Drive-created cases carry a date of accident, so this column
   * is the one the Filevine import most needs to land. Filevine's API calls it
   * `incidentDate`; the rest are the ordinary PI synonyms.
   */
  for (const h of [
    'DOA', 'Incident Date', 'incident date', 'Date of Incident',
    'Date of Loss', 'Loss Date', 'Incident Date (Loss)',
  ]) {
    const [col] = mapHeaders([h]);
    assert.equal(col.key, 'doa', `${h} should map to doa, got "${col.key}"`);
  }
});

test('the accident-date spellings do not collide with Date Opened', () => {
  // Both start with "DATE O", and the match is a prefix match -- so this is
  // the collision that would silently file an accident date as an open date.
  assert.equal(mapHeaders(['Date Opened'])[0].key, 'openDate');
  assert.equal(mapHeaders(['DATE OPENED'])[0].key, 'openDate');
  assert.equal(mapHeaders(['Date of Loss'])[0].key, 'doa');
});
