...
const EXCEL_MAX = 60000; // 2064-04-08
 ...
 */
/**
 * Pull a case number out of a combined project name.
 *
 * Filevine names a project "Rivera, Marcus 26-033" and every one of its
 * reports exports that single string. There is no separate case-number
 * column in ANY of them — so without this, importing the firm's own export
 * gives 359 cases with auto-allocated numbers that match nothing: not the
 * client's emails, not the medical authorisations, not the Drive folders,
 * not the numbers staff say out loud. The number IS the identifier; losing
 * it is losing the ability to match a second report to the first.
 *
 * 353 of the firm's 359 names carry one. The other six are handled by
 * returning the name unchanged rather than inventing anything.
 *
 * The LAST match wins. Filevine appends qualifiers after the number
 * ("Scott, Ashley 23-155 et al", "Test, Test 2nd Case"), and a client whose
 * name happens to contain digits should not beat the number at the end.
 *
 * @returns {{ clientName: string, caseNumber: string }}
 */
export function splitProjectName(raw) {
  const text = String(raw ?? '').trim();
  if (!text) return { clientName: '', caseNumber: '' };

  // Anchored on word boundaries so "100007504" cannot yield "00-075".
  const matches = [...text.matchAll(/(?<![\d-])(\d{2}-\d{3})(?![\d-])/g)];
  if (!matches.length) return { clientName: text, caseNumber: '' };

  const last = matches[matches.length - 1];
  const caseNumber = last[1];

  // Everything except the number, tidied. Trailing qualifiers are KEPT --
  // "et al" and "2nd Case" distinguish two matters for the same client, and
  // dropping them would merge records that must stay apart.
  const clientName = (text.slice(0, last.index) + ' ' + text.slice(last.index + caseNumber.length))
    .replace(/\s+/g, ' ')
    .replace(/\s*[-–—,]\s*$/, '')
    .trim();

  return { clientName: clientName || text, caseNumber };
}
...
export function planImport({
  ...
  rows.forEach((row, rowIndex) => {
    ...
    /*
     * `internalId` is a matching key, not data. It comes from this app's own
     * export and is removed before anything is written -- left in `values` it
     * would land in the JSONB tail as a field nothing reads.
     */
    const internalId = String(values.internalId || '').trim();
    delete values.internalId;

    /*
     * A combined "Name" column carries both. Only consulted when the file
     * has no case-number column of its own, so a spreadsheet that has one
     * always wins -- an explicit column is a stated intention and a number
     * scraped out of a name is an inference.
     */
    let clientName = (values.clientName || '').trim();
    if (!String(values.caseNumber || '').trim() && clientName) {
      const split = splitProjectName(clientName);
      if (split.caseNumber) {
        clientName = split.clientName;
        values.clientName = split.clientName;
        values.caseNumber = split.caseNumber;
      }
    }
    ...
    /*
     * Match on the internal id first. It is exact, and it is the only thing
     * that can match a case carrying no case number -- which is most of them
     * while the firm is still rebuilding its list.
     */
    const existingId = (internalId && matters[internalId] ? internalId : undefined)
      ?? (ck ? byCase.get(ck) : undefined);
    ...
    if (existingId) {
      entries.push({
        ...
        action: 'update',
        matterId: existingId,
        reason: internalId && matters[internalId]
          ? `Matches an existing case in this system${values.caseNumber ? ` (${values.caseNumber})` : ''}`
          : `Matches existing case ${values.caseNumber}`,
