...
import { daysFromToday, todayInFirmTz, urgency, instantToFirmDate } from './dates.js';
...
export function daysSinceActivity(matter, today = todayInFirmTz()) {
  ...
  if (stamp) {
    // The stamp is UTC; `today` is Central. Slicing the first ten characters
    // would compare a UTC calendar date against a firm-timezone one, which
    // reads "-1d ago" every evening after 7pm CT. See instantToFirmDate.
    const iso = instantToFirmDate(stamp);
    const d = daysFromToday(iso, today);
    return d === null ? null : 0 - d;
  }
