 ...
 */
/**
 * Who a generated deadline belongs to.
 *
 * A person already named on the task wins. Otherwise the matter's attorney,
 * and only then the absence of one -- which is a state, not a name.
 */
function assigneeFor(existing, values) {
  const claimed = String(existing?.assignedTo || '').trim();
  if (claimed && claimed.toLowerCase() !== UNASSIGNED.toLowerCase()) return claimed;
  return String(values?.attorney || '').trim() || UNASSIGNED;
}
  ...
  for (const [matterId, matter] of Object.entries(matters || {})) {
    ...
    for (const rule of CHAIN_RULES) {
      ...
      tasks[id] = {
        ...
        manualOverride,
        /*
         * ⚠️ "Unassigned" IS A TRUTHY STRING.
         *
         * This read `existing?.assignedTo || values.attorney`, so a task
         * already carrying the literal "Unassigned" kept it forever -- filling
         * in the attorney afterwards changed nothing, and every deadline the
         * system had ever generated stayed on nobody's list.
         *
         * An explicit assignment is still honoured: a task somebody put on a
         * particular person's desk is not reassigned when the attorney field
         * changes. Only the ones nobody has claimed follow the attorney.
         */
        assignedTo: assigneeFor(existing, values),
