 ...
 */
/**
 * Deadline labels for the calendar, read from the registry rather than
 * retyped. Two lists of the same fifteen names would drift, and the drift
 * would show up as a calendar entry called something nobody recognises.
 */
const DCO_LABELS = Object.fromEntries(
  (SECTION_BY_KEY.dco?.groups || [])
    .flatMap((g) => g.fields || [])
    .map((f) => [f.key, f.label]),
);
...
export function collectEvents({ matters = {}, tasks = {}, sections = {}, kinds = null } = {}) {
  ...
  for (const [id, matter] of Object.entries(matters)) {
    ...
    /*
     * ⚠️ TWO SHAPES, AND BOTH HAVE TO BE READ.
     *
     * The DCO tab is now a fixed checklist -- fifteen named deadlines stored
     * as `datedone` FIELDS -- and it keeps a free-form table for whatever
     * else a judge put on the order, stored as ROWS.
     *
     * Reading only the rows is exactly the bug fixed on 2026-09-18, where
     * every DCO deadline was invisible to the calendar. Changing the tab's
     * shape without changing this would have reintroduced it for the
     * fifteen deadlines that matter most.
     */
    const dco = sections?.[id]?.dco;

    for (const [key, value] of Object.entries(dco?.fields || {})) {
      const due = typeof value === 'object' && value ? value.dateValue : null;
      if (!due) continue;
      out.push({
        id: `${id}:dco:${key}`,
        kind: 'dco',
        date: due,
        title: DCO_LABELS[key] || 'Docket deadline',
        matterId: id,
        rank: 2,
        tone: 'orange',
        // A deadline already met still belongs on the calendar -- it is a
        // record of the date, not only a thing outstanding.
        done: Boolean(value.doneDate),
      });
    }

    for (const row of dco?.rows || []) {
