...
import {
  nameKey, caseNumberIn, proposeMatch, planSync, looseCaseNumberIn,
  planCreateFromFolders, planFolderCreate,
} from './drive-match.js';
...
test('every case gets the same subfolder shape', () => {
  /*
   * "Medicals", not "Medical Records". The firm has filed into a folder called
   * Medicals for years; provisioning the other name gave a new case both --
   * one made here, one made by the first upload -- and split its documents.
   */
  assert.ok(CASE_SUBFOLDERS.includes('Medicals'));
  assert.ok(!CASE_SUBFOLDERS.includes('Medical Records'), 'the old name came back');
...
test('no two subfolders differ only by case or spacing', () => {
  // Drive matching folds case and trims, so two such names would resolve to
  // one folder -- two entries in the template, one folder on disk, and a
  // provisioning step that silently does nothing the second time.
  const folded = CASE_SUBFOLDERS.map((f) => f.trim().toLowerCase());
  assert.equal(new Set(folded).size, folded.length, CASE_SUBFOLDERS.join(', '));
});
...
/* ------------------------------------------------------------------ *
 * planCreateFromFolders — Drive as the case list
 * ------------------------------------------------------------------ */

const f = (id, name) => ({ id, name });

test('a folder with a case number and no matter becomes a case', () => {
  const { create, skipped } = planCreateFromFolders([f('f1', 'Rivera, Marcus 26-033')], {});
  assert.equal(skipped.length, 0);
  assert.deepEqual(
    { clientName: create[0].clientName, caseNumber: create[0].caseNumber },
    { clientName: 'Rivera, Marcus', caseNumber: '26-033' },
  );
});

test('TEMPLATES and the other housekeeping folders are passed over', () => {
  // The named reason this exists. These sit beside the case folders and
  // must never become cases.
  const { create, skipped } = planCreateFromFolders(
    [f('f1', 'TEMPLATES'), f('f2', 'ARCHIVED FILES'), f('f3', 'FILEVINE REPORTS AS OF 8-30-26')],
    {},
  );
  assert.equal(create.length, 0);
  assert.equal(skipped.length, 3);
  for (const s2 of skipped) assert.match(s2.reason, /no case number/);
});

test('a real case whose folder lacks a number is also passed over, not guessed at', () => {
  // "Jannesari, Mohammad" is a real case folder with no number. Inventing
  // one would put a case in the system under a number that means nothing.
  const { create, skipped } = planCreateFromFolders([f('f1', 'Jannesari, Mohammad')], {});
  assert.equal(create.length, 0);
  assert.match(skipped[0].reason, /no case number/);
});

test('an existing case number is left for the sync to link', () => {
  const matters = { m1: { values: { clientName: 'Rivera, Marcus', caseNumber: '26-033' } } };
  const { create, skipped } = planCreateFromFolders([f('f1', 'Rivera, Marcus 26-033')], matters);
  assert.equal(create.length, 0);
  assert.match(skipped[0].reason, /already exists/);
});

test('a folder already linked is passed over even if its number looks free', () => {
  const matters = {
    m1: { values: { clientName: 'Someone', caseNumber: '26-999' }, driveFolderId: 'f1' },
  };
  const { create, skipped } = planCreateFromFolders([f('f1', 'Rivera, Marcus 26-033')], matters);
  assert.equal(create.length, 0);
  assert.match(skipped[0].reason, /already linked/);
});

test('TWO FOLDERS SHARING A NUMBER CREATE NOTHING', () => {
  /*
   * The firm has five such pairs. Creating from the first and skipping the
   * second is an arbitrary decision about which folder is the real case —
   * and the second would fail the unique index anyway, after the first had
   * already been written.
   */
  const { create, skipped } = planCreateFromFolders(
    [f('f1', 'Aguilar, Carlos 23-180'), f('f2', 'Aguilar, Carlos 23-180 Declaratory Action')],
    {},
  );
  assert.equal(create.length, 0);
  assert.equal(skipped.length, 2);
  assert.match(skipped[0].reason, /2 folders carry 23-180/);
});

test('an archived matter still holds its case number', () => {
  /*
   * ⚠️ THIS TEST USED TO CONTRADICT ITSELF. Its comment read "a folder for an
   * archived case should link to it rather than spawn a duplicate -- so the
   * number still counts as in use", and then it asserted create.length === 1,
   * which is the duplicate. The right rule was written down and the wrong
   * outcome was pinned.
   *
   * The firm hit it: archiving a case made the sync offer to create a second
   * one from its folder, carrying the same number. 26-033 means "the 33rd
   * case opened in 2026"; two cases holding it makes the number meaningless.
   */
  const matters = {
    m1: { values: { clientName: 'Rivera, Marcus', caseNumber: '26-033' }, archivedAt: 'x' },
  };
  const { create, skipped } = planCreateFromFolders([f('f1', 'Rivera, Marcus 26-033')], matters);
  assert.equal(create.length, 0, 'offered to create a duplicate of an archived case');
  assert.match(skipped[0].reason, /archived/i, 'the reason must say the case is archived');
});

test("an archived case's folder stays linked to it", () => {
  // The documents in that folder belong to that matter. Archiving the case
  // does not make them nobody's.
  const matters = {
    m1: { values: { clientName: 'Rivera, Marcus', caseNumber: '26-033' }, driveFolderId: 'f1', archivedAt: 'x' },
  };
  const { create, skipped } = planCreateFromFolders([f('f1', 'Rivera, Marcus 26-033')], matters);
  assert.equal(create.length, 0);
  assert.match(skipped[0].reason, /already linked/i);

  const match = proposeMatch(f('f1', 'Rivera, Marcus 26-033'), matters);
  assert.equal(match.status, 'linked');
  assert.equal(match.matterId, 'm1');
  assert.equal(match.archived, true, 'the caller should be able to say it is archived');
});

test('an archived case is not a candidate for an UNLINKED folder', () => {
  // The other half of the rule. Its own folder stays linked; a new folder
  // must not be auto-attached to a case nobody works any more.
  const matters = {
    m1: { values: { clientName: 'Rivera, Marcus', caseNumber: '26-033' }, archivedAt: 'x' },
  };
  const match = proposeMatch(f('f9', 'Rivera, Marcus'), matters);
  assert.notEqual(match.status, 'auto', 'auto-linked a folder to an archived case');
});

test('a folder named only as a number still gets a findable name', () => {
  const { create } = planCreateFromFolders([f('f1', '26-033')], {});
  assert.equal(create[0].clientName, '26-033');
  assert.equal(create[0].caseNumber, '26-033');
});

test('the derived name matches what the spreadsheet importer would derive', () => {
  /*
   * The two paths must agree, or importing the Filevine reports afterwards
   * would create a second copy of every case instead of updating it.
   */
  const name = 'Baker, Susan 26-046 1st Case';
  const { create } = planCreateFromFolders([f('f1', name)], {});
  const viaImport = splitProjectName(name);
  assert.equal(create[0].clientName, viaImport.clientName);
  assert.equal(create[0].caseNumber, viaImport.caseNumber);
});

/* ------------------------------------------------------------------ *
 * planFolderCreate — the review queue's third resolution
 * ------------------------------------------------------------------ */

test('a folder whose case number is free creates a case with that number', () => {
  const p = planFolderCreate('Aguilar, Carlos 23-180', {});
  assert.equal(p.caseNumber, '23-180');
  assert.equal(p.clientName, 'Aguilar, Carlos');
  assert.equal(p.useNewNumber, false, 'the folder\'s own number should be used');
  assert.equal(p.holderId, '');
});

test('a folder whose number is taken becomes its own case, related to the holder', () => {
  /*
   * The real pair from the firm's Drive. Only one case can hold 23-180
   * (matter_case_number_uq) and only one case can hold a folder, so the second
   * folder cannot reuse the number and cannot attach to the first case. It
   * becomes its own case on a fresh number, related back.
   */
  const matters = { m1: { values: { caseNumber: '23-180', clientName: 'Aguilar, Carlos' } } };
  const p = planFolderCreate('Aguilar, Carlos 23-180 Progressive Declaratory Action', matters);
  assert.equal(p.caseNumber, '23-180');
  assert.equal(p.holderId, 'm1', 'should relate back to the case holding 23-180');
  assert.equal(p.useNewNumber, true, 'must not try to reuse a taken number');
});

test('a folder with no case number in its name still creates a case', () => {
  // "Hopkins J 15", "Jannesari, Mohammad" -- real folders the create step
  // passed over entirely, leaving no way to bring them in.
  for (const name of ['Hopkins J 15', 'Jannesari, Mohammad', 'Perdue, Bobby 193']) {
    const p = planFolderCreate(name, {});
    assert.equal(p.caseNumber, '', name);
    assert.equal(p.useNewNumber, true, `${name} should take the next free number`);
    assert.equal(p.holderId, '');
    assert.ok(p.clientName, `${name} must still carry a client name`);
  }
});

test('an archived case is still the holder of its number', () => {
  /*
   * REVERSED DELIBERATELY. The old rule was "otherwise a folder is blocked
   * forever by a case nobody works any more" -- a real worry, but the answer
   * to it is to UNARCHIVE that case, not to mint a second one carrying its
   * number. Nothing is blocked forever: planCreateFromFolders now says
   * exactly that in its skip reason.
   *
   * Reusing the number is the outcome that cannot be undone by a click.
   */
  const matters = { m1: { archivedAt: '2026-01-01', values: { caseNumber: '23-180' } } };
  const p = planFolderCreate('Aguilar, Carlos 23-180', matters);
  assert.equal(p.holderId, 'm1', 'the archived case is the holder');
  assert.equal(p.useNewNumber, true, 'a new case here must take the next free number');
});

test('the client name never comes back empty', () => {
  // matter.client_name is NOT NULL, and a case nobody can find by name is
  // worse than an ugly one.
  assert.equal(planFolderCreate('26-033', {}).clientName, '26-033');
  assert.equal(planFolderCreate('   ', {}).clientName, '');
  assert.equal(planFolderCreate(undefined, {}).clientName, '');
});

/* ------------------------------------------------------------------ *
 * A skip reason has to describe what the sync will actually do
 * ------------------------------------------------------------------ */

test('a number on two folders is not promised a link, even when the case exists', () => {
  /*
   * 26-063, reported by the firm. Two folders carry it AND the case exists,
   * so the old branch order caught it on "already exists — the sync links
   * it". The sync then did NOT link it, because planSync refuses to
   * auto-link a number carried by more than one folder: it cannot tell which
   * folder is the case, and guessing files one client's records under
   * another's matter.
   *
   * Somebody read the message, ran the sync, and watched nothing happen. A
   * skip reason is the only explanation anyone gets.
   */
  const matters = {
    m1: { values: { clientName: 'Smithers, Jr., Timothy', caseNumber: '26-063' } },
  };
  const folders = [
    f('f1', 'Smithers, Jr., Timothy 26-063'),
    f('f2', 'Smithers, Jr., Timothy 26-063 Second Action'),
  ];
  const { create, skipped } = planCreateFromFolders(folders, matters);

  assert.equal(create.length, 0);
  for (const s of skipped) {
    assert.match(s.reason, /2 folders carry 26-063/, s.reason);
    assert.match(s.reason, /review queue/i, 'it must say where the decision happens');
    assert.doesNotMatch(s.reason, /the sync links it/, 'promised a link it will not make');
  }
});

test('and the sync itself does send both of those folders to review', () => {
  // The other half of the same claim: if planSync ever started auto-linking
  // a colliding number, the message above would become the lie instead.
  const matters = {
    m1: { values: { clientName: 'Smithers, Jr., Timothy', caseNumber: '26-063' } },
  };
  const plan = planSync(
    [f('f1', 'Smithers, Jr., Timothy 26-063'), f('f2', 'Smithers, Jr., Timothy 26-063 Second Action')],
    matters,
  );
  assert.equal(plan.auto.length, 0, 'a colliding number must never auto-link');
  assert.ok(plan.ambiguous.length + plan.unmatched.length >= 1);
});

test('a number on ONE folder whose case exists is still promised a link', () => {
  // The ordinary case must keep its ordinary message.
  const matters = { m1: { values: { clientName: 'Rivera, Marcus', caseNumber: '26-033' } } };
  const { skipped } = planCreateFromFolders([f('f1', 'Rivera, Marcus 26-033')], matters);
  assert.match(skipped[0].reason, /already exists — the sync links it/);
});
