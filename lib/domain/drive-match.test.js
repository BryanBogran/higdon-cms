...
test('an archived matter does not reserve its case number', () => {
  // Archive is reversible, but a folder for an archived case should link to
  // it rather than spawn a duplicate -- so the number still counts as in use.
  const matters = {
    m1: { values: { clientName: 'Rivera, Marcus', caseNumber: '26-033' }, archivedAt: 'x' },
  };
  const { create } = planCreateFromFolders([f('f1', 'Rivera, Marcus 26-033')], matters);
  assert.equal(create.length, 1, 'an archived case is out of the live list');
});
...
test('an archived case does not reserve its number', () => {
  // Otherwise a folder is blocked forever by a case nobody works any more.
  const matters = { m1: { archivedAt: '2026-01-01', values: { caseNumber: '23-180' } } };
  const p = planFolderCreate('Aguilar, Carlos 23-180', matters);
  assert.equal(p.holderId, '');
  assert.equal(p.useNewNumber, false);
});
