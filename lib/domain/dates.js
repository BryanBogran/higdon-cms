...
/**
 * The firm-timezone calendar date of an INSTANT (a timestamp), as "YYYY-MM-DD".
 *
 * `todayInFirmTz` above does this for now; this is the same conversion for a
 * stored timestamp, named so the call site reads correctly.
 *
 * The reason it exists: a timestamp column is UTC, so `stamp.slice(0, 10)`
 * yields the UTC calendar date. Compared against a firm-timezone "today" that
 * is off by one for the five hours between 7pm Central and midnight UTC --
 * which is when a matter touched seconds ago reports "-1d ago". Same off-by-one
 * this file was written to eliminate, on the timestamp side rather than the
 * date-only side.
 *
 * Returns null for anything unparseable, never NaN, per rule 3 above.
 */
export function instantToFirmDate(value) {
  if (!value) return null;
  const d = value instanceof Date ? value : new Date(value);
  if (Number.isNaN(d.getTime())) return null;
  return todayInFirmTz(d);
}
...
/**
 * Long display: "September 23, 2026". For documents, not for tables.
 *
 * ⚠️ THE FALLBACK IS EMPTY, NOT `fmt`'s EM DASH. An em dash on the "Date of
 * Birth:" line of a letter to a medical provider reads as a redaction — as
 * though the firm withheld it — rather than as a field nobody filled in. A
 * blank at least reads as an oversight, and the caller is expected to have
 * refused to generate the document at all before it gets this far.
 *
 * `timeZone: 'UTC'` for the reason `fmt` gives: the timestamp built from the
 * parsed parts is already the intended calendar day, so rendering it in any
 * other zone would move it.
 */
