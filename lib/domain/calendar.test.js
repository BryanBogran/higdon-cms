...
/* ------------------------------------------------------------------ *
 * DCO deadlines reach the calendar
 * ------------------------------------------------------------------ */

const dcoMatters = { m1: { values: { clientName: 'Aguilar, Carlos', caseNumber: '23-180' } } };
const dcoSections = {
  m1: {
    dco: {
      rows: [
        { id: 'r1', deadline: 'Expert designation', date: '2026-10-15' },
        { id: 'r2', deadline: 'Discovery cutoff', date: '2026-11-20' },
        { id: 'r3', deadline: 'No date yet', date: '' },
      ],
    },
  },
};

test('deadlines typed on the DCO tab appear on the calendar', () => {
  /*
   * THE BUG. collectEvents took only `matters` and `tasks`, so every deadline
   * transcribed off a docket control order was write-only -- stored, rendered
   * on its own tab, and read by nothing. The single `dco` DATE on Case Info
   * did calendar, which is what hid it: a "DCO" chip appeared, just not the
   * deadlines underneath it.
   *
   * A court-ordered deadline that silently fails to appear is the worst shape
   * this system can take.
   */
  const events = collectEvents({ matters: dcoMatters, tasks: {}, sections: dcoSections });
  const dco = events.filter((e) => e.kind === 'dco');
  assert.equal(dco.length, 2, 'both dated rows should appear');
  assert.deepEqual(dco.map((e) => e.title).sort(), ['Discovery cutoff', 'Expert designation']);
  assert.deepEqual(dco.map((e) => e.date).sort(), ['2026-10-15', '2026-11-20']);
});

test('each DCO row carries its own name, not a generic label', () => {
  // The reason for transcribing them one per row instead of keeping one date.
  const e = collectEvents({ matters: dcoMatters, tasks: {}, sections: dcoSections })
    .find((x) => x.date === '2026-10-15');
  assert.equal(e.title, 'Expert designation');
  assert.equal(e.matterId, 'm1');
  assert.equal(e.tone, 'orange');
});

test('a DCO row with no name still shows, rather than being hidden', () => {
  // Hiding a dated deadline because somebody skipped the label would be the
  // same class of failure this fixed.
  const sections = { m1: { dco: { rows: [{ id: 'r9', deadline: '   ', date: '2026-12-01' }] } } };
  const e = collectEvents({ matters: dcoMatters, tasks: {}, sections })
    .find((x) => x.kind === 'dco');
  assert.equal(e.title, 'Docket deadline');
});

test('DCO rows sit under the existing DCO filter, with the Case Info date', () => {
  // Staff think of both as "the DCO"; two chips would mean ticking two boxes
  // to see one court order.
  const matters = { m1: { values: { clientName: 'A', caseNumber: '1', dco: '2026-09-30' } } };
  const shown = collectEvents({ matters, tasks: {}, sections: dcoSections, kinds: ['dco'] });
  assert.equal(shown.length, 3, 'the Case Info date plus both dated rows');
  const hidden = collectEvents({ matters, tasks: {}, sections: dcoSections, kinds: ['sol'] });
  assert.equal(hidden.filter((e) => e.kind === 'dco').length, 0);
});

test('an archived matter contributes no DCO deadlines', () => {
  // Same rule the matter dates already follow: an old deadline reads as live.
  const matters = { m1: { archivedAt: '2026-01-01', values: { clientName: 'A' } } };
  const events = collectEvents({ matters, tasks: {}, sections: dcoSections });
  assert.equal(events.filter((e) => e.kind === 'dco').length, 0);
});

test('no sections passed means no DCO rows, and no crash', () => {
  const events = collectEvents({ matters: dcoMatters, tasks: {} });
  assert.equal(events.filter((e) => e.kind === 'dco').length, 0);
});

/* ------------------------------------------------------------------ *
 * DCO: a checklist of fields AND a table of rows
 * ------------------------------------------------------------------ */

test('a deadline from the DCO checklist reaches the calendar', () => {
  /*
   * ⚠️ THE REGRESSION GUARD. On 2026-09-18 every DCO deadline was invisible
   * to the calendar because this only read `rows`. The tab is now a fixed
   * checklist stored as FIELDS, so reading only rows would have brought the
   * same bug back for the fifteen deadlines that matter most.
   */
  const events = collectEvents({
    matters: dcoMatters,
    sections: {
      m1: { dco: { fields: { discoverydeadline: { dateValue: '2027-03-01', doneDate: '' } }, rows: [] } },
    },
  });
  const dco = events.filter((e) => e.kind === 'dco');
  assert.equal(dco.length, 1);
  assert.equal(dco[0].date, '2027-03-01');
  assert.equal(dco[0].title, 'Discovery Deadline', 'the label must come from the registry');
});

test('the free-form table still reaches it too', () => {
  // A judge can put anything on an order; those live in rows.
  const events = collectEvents({
    matters: dcoMatters,
    sections: {
      m1: {
        dco: {
          fields: { trial: { dateValue: '2027-09-28' } },
          rows: [{ id: 'r1', deadline: 'Rule 11 agreement due', date: '2027-04-15' }],
        },
      },
    },
  });
  const titles = events.filter((e) => e.kind === 'dco').map((e) => e.title).sort();
  assert.deepEqual(titles, ['Rule 11 agreement due', 'Trial']);
});

test('a checklist item with no date is not an event', () => {
  const events = collectEvents({
    matters: dcoMatters,
    sections: { m1: { dco: { fields: { trial: { dateValue: '', doneDate: '' }, docketcall: {} }, rows: [] } } },
  });
  assert.deepEqual(events.filter((e) => e.kind === 'dco'), []);
});

test('a deadline already met still appears, marked done', () => {
  // It is a record of the date, not only of something outstanding.
  const events = collectEvents({
    matters: dcoMatters,
    sections: { m1: { dco: { fields: { docketcall: { dateValue: '2026-07-24', doneDate: '2026-07-24' } }, rows: [] } } },
  });
  const [e] = events.filter((x) => x.kind === 'dco');
  assert.ok(e);
  assert.equal(e.done, true);
});

test('every checklist key has a label the calendar can print', () => {
  // A key with no label shows as "Docket deadline" -- fine as a fallback for
  // an unnamed row, wrong for a named field somebody filled in.
  const fields = (SECTION_BY_KEY.dco?.groups || []).flatMap((g) => g.fields || []);
  assert.ok(fields.length >= 15, `expected the full order, got ${fields.length}`);
  for (const f of fields) {
    const events = collectEvents({
      matters: dcoMatters,
      sections: { m1: { dco: { fields: { [f.key]: { dateValue: '2027-01-01' } }, rows: [] } } },
    });
    const [e] = events.filter((x) => x.kind === 'dco');
    assert.equal(e.title, f.label, `${f.key} printed "${e.title}"`);
  }
});
