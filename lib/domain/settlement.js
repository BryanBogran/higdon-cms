 ...
 */
export function buildLines(rows = [], { labelKeys = [], amountKey, reductions = {} } = {}) {
  const lines = [];
  let originalCents = 0;
  let reducedCents = 0;
  let missing = 0;
  ...
  for (const row of rows) {
    const original = parseMoney(row?.[amountKey]);
    if (original === null) {
      missing++;
      continue;
    }
    ...
    lines.push({ id: row.id, label: String(label), originalCents: original, percent, reducedCents: reduced });
    originalCents += original;
    reducedCents += reduced;
  }

  return { lines, originalCents, reducedCents, missing, count: rows.length };
}
