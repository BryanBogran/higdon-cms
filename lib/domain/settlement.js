...
/**
 * Integer cents as a bare amount for a text box: 123450 -> "1234.50".
 *
 * ⚠️ FOR A PLACEHOLDER OR A READ-ONLY DISPLAY ONLY. Never the `value` of an
 * input somebody is typing in -- rendering a parsed number back into a
 * controlled input is what made cents untypable on the settlement
 * calculator: the decimal point was erased on the keystroke that made it,
 * and $1,234.50 became $12,345.00.
 *
 * `String(cents / 100)` was what the placeholder used, and it drops the
 * trailing zero -- $1,234.50 suggested "1234.5", which reads as a different
 * number to somebody checking a settlement sheet.
 */
