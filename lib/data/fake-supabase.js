 ...
 */
export function fakeSupabase(seed = {}, { rpc = {}, failOn = [], maxRows = null } = {}) {
  const tables = {};
  ...
  function builder(table) {
    ...
    const run = () => {
      ...
      if (op === 'select') {
        ...
        /*
         * ⚠️ THE SERVER-SIDE ROW CAP, simulated.
         *
         * PostgREST refuses to return more than `max-rows` for one request
         * and says nothing about it -- no error, no flag, just fewer rows
         * than exist. On the firm's database that cap is 1000 against 1995
         * section rows, so 45% of their medical records, expenses and
         * provider bills were in the database and had never once been
         * fetched. A query ordered by `ordinal` lost every row from ordinal
         * 3 up, for every case at once.
         *
         * Modelling it here is the only way a test can prove the paging
         * actually pages, rather than asserting against a mock that would
         * hand back a million rows quite happily.
         */
        const start = rangeReq ? rangeReq.from : 0;
        const requested = rangeReq ? rangeReq.to - rangeReq.from + 1 : Infinity;
        const allowed = Math.min(requested, maxRows ?? Infinity);
        data = data.slice(start, start + (allowed === Infinity ? data.length : allowed));
      }
    ...
    const api = {
      ...
      /*
       * Orders ACCUMULATE. PostgREST applies them in the order given, and a
       * second key is what makes pagination safe -- without a total order,
       * two pages of the same query can overlap or skip rows entirely.
       */
      order(column, opts) { orders.push({ column, ascending: opts?.ascending !== false }); return api; },
      range(from, to) { rangeReq = { from, to }; return api; },
      limit() { return api; },
  ...
  return {
    ...
    async rpc(name, args = {}) {
      ...
      // An explicit stub always wins, so a test can still force an error.
      if (name in rpc) {
      ...
      /*
       * The two section writers are EMULATED rather than stubbed, because
       * what matters about them is the end state of the table -- that a
       * second writer merges onto the first instead of replacing it. A
       * canned return value would assert nothing about the thing that
       * broke. See supabase/017.
       */
      if (name === 'set_section_field') {
        const t = (tables.matter_section_data ||= []);
        const row = t.find(
          (r) => r.matter_id === args.p_matter_id && r.section_key === args.p_section_key
        );
        if (row) {
          // jsonb `||` -- merge onto whatever is in the table RIGHT NOW.
          row.fields = { ...(row.fields || {}), [args.p_field_key]: args.p_value };
          row.updated_at = new Date().toISOString();
        } else {
          t.push({
            matter_id: args.p_matter_id,
            section_key: args.p_section_key,
            fields: { [args.p_field_key]: args.p_value },
      ...
      if (name === 'update_section_row') {
        const t = tables.matter_section_row || [];
        const row = t.find((r) => r.id === args.p_row_id);
        // No row -> no id back. This is the whole point: an update that
        // matches nothing must be distinguishable from one that worked.
        if (!row) return { data: null, error: null };
        row.data = { ...(row.data || {}), ...(args.p_patch || {}) };
        delete row.data.id;
