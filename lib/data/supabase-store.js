...
export function createSupabaseStore() {
  ...
  return {
    ...
    async loadAll() {
      const [matters, checklist, activity, sectionData, sectionRows, profiles] = await Promise.all([
      ...
      return {
        ...
        tasks: Object.fromEntries(
          all.filter((r) => r.kind === 'task' && r.source === 'auto')
            .map((r) => [r.id, taskRowToTask(r)])
        ),
        activity: Object.fromEntries(
          all.filter((r) => !(r.kind === 'task' && r.source === 'auto'))
            .map((r) => [r.id, activityRowToEntry(r)])
        ),
        sections,
