...
import { assignablePeople } from '@/lib/domain/team';
import { FIELDS, DOC_FIELDS, emptyValues, emptyDocValue, authorDisplay } from '@/lib/domain/fields';
...
export function rowsToMatters(matterRows, checklistRows) {
  ...
  for (const row of matterRows || []) {
    ...
    out[row.id] = {
      ...
      /*
       * ⚠️ THE SAME OMISSION, A SECOND TIME.
       *
       * linkClientContact writes client_contact_id and this did not read it
       * back, so linking a contact card held until the page was refreshed and
       * then silently came undone -- staff relinked the same card every visit.
       *
       * The write was never the problem. A column that is written and not
       * mapped here looks exactly like a save that failed, which is why the
       * two lines above carry the same warning about drive_folder_id.
       *
       * ANY new column on `matter` that is not in COLUMN_OF has to be added
       * here by hand, because nothing else will notice.
       */
...
export function activityRowToEntry(row) {
  return {
    ...
    title: row.title || '',
    body: row.body || '',
    author: authorDisplay(row.author_label, row.source),
    ...
    source: row.source,
    createdAt: row.created_at,
    // Set when this row is a REPLY to another entry. loadAll partitions on it
    // so a reply never reaches the feed as a post of its own.
    parentId: row.parent_id || null,
  };
}

/**
 * Replies grouped under the entry they answer: `{ [parentId]: [entry, ...] }`.
 *
 * Oldest first -- a thread is read top to bottom, unlike the feed, which is
 * newest first because it is a list of what just happened rather than a
 * conversation.
 */
export function repliesByParent(rows = []) {
  const out = {};
  for (const row of rows) {
    if (!row?.parent_id) continue;
    (out[row.parent_id] ||= []).push(activityRowToEntry(row));
  }
...
const CONTACT_COLUMNS = {
  ...
  // The State Bar number. Attorneys only in practice, but stored on every
  // contact rather than in a separate table for one field.
...
/* ------------------------------------------------------------------ *
 * Reading a whole table, without being quietly given part of one
 * ------------------------------------------------------------------ */

/** PostgREST's own page size. Asking for more in one request achieves nothing. */
const PAGE_SIZE = 1000;

/**
 * Refuse to loop forever. 100 pages is 100,000 rows -- far past anything this
 * firm will hold, so reaching it means something is wrong rather than large.
 */
const MAX_PAGES = 100;
 ...
 * ── Why this exists ──────────────────────────────────────────────────────
 *
 * `select('*')` DOES NOT RETURN EVERY ROW. PostgREST caps a response at
 * `max-rows` -- 1000 on this project -- and says nothing whatsoever about it.
 * No error, no flag, no truncation notice. The client is simply handed fewer
 * rows than exist and has no way to know.
 *
 * On 2026-09-18 the firm reported medical records disappearing from a case.
 * They had not. `matter_section_row` held 1,995 rows against a cap of 1,000,
 * and the query was ordered by `ordinal` ACROSS EVERY CASE AT ONCE -- so the
 * response contained all the ordinal-0 rows, all the ordinal-1s, part of the
 * ordinal-2s, and then stopped. Every case in the firm silently lost its
 * fourth row onwards, and 901 rows -- 45% of all section data -- had never
 * once been fetched since the day they were written.
 *
 * It presented as data loss because from a user's seat it is indistinguishable
 * from data loss.
 *
 * ── ⚠️ THE ORDER MUST BE TOTAL ───────────────────────────────────────────
 *
 * Paging depends on the database returning rows in the same sequence every
 * time. `order('ordinal')` alone does not: rows sharing an ordinal may come
 * back in any order, so page 2 can repeat a row from page 1 and skip another
 * entirely. Every caller below therefore orders by something UNIQUE last --
 * the primary key.
 *
 * That is not a detail. Pagination on a non-total order loses rows, which is
 * the bug this function exists to fix, reintroduced in a subtler form.
 *
 * @param makeQuery a FACTORY, not a query -- a Supabase builder is single-use
 *                  and cannot be re-ranged.
 */
async function fetchAllRows(makeQuery) {
  ...
  for (let page = 0; page < MAX_PAGES; page++) {
    ...
    const batch = data || [];
    out.push(...batch);
    // A short page is the last page. This is the only reliable end signal:
    // asking for a count costs another query and can disagree under writes.
    if (batch.length < PAGE_SIZE) return { data: out, error: null };
  }
  /*
   * Loud, not silent. Returning what we have would be the original bug --
   * a partial answer presented as a complete one.
   */
  return {
    data: null,
    error: new Error(
      `Refusing to page past ${MAX_PAGES * PAGE_SIZE} rows. Something is wrong with this query.`
    ),
  };
}
...
export function createSupabaseStore(client) {
  ...
  return {
    ...
    async loadAll() {
      /*
       * Every one of these is paged, and every one ends its ordering on a
       * UNIQUE column. See fetchAllRows -- the cap is silent and the order
       * has to be total or paging itself loses rows.
       */
      const [matters, checklist, activity, sectionData, sectionRows, profiles, relations, contacts] =
        await Promise.all([
        fetchAllRows(() =>
          db.from('matter').select('*')
            .order('last_activity_at', { ascending: false }).order('id')),
        fetchAllRows(() =>
          db.from('matter_checklist_item').select('*')
            .order('matter_id').order('field_key')),
        fetchAllRows(() =>
          db.from('activity').select('*')
            .order('created_at', { ascending: false }).order('id')),
        fetchAllRows(() =>
          db.from('matter_section_data').select('*')
            .order('matter_id').order('section_key')),
        fetchAllRows(() =>
          db.from('matter_section_row').select('*')
            .order('matter_id').order('section_key').order('ordinal').order('id')),
        fetchAllRows(() => db.from('profile').select('*').order('id')),
        // Related cases. Tolerated as missing until 005_sections.sql runs.
        fetchAllRows(() => db.from('matter_relation').select('*').order('id')),
        ...
        fetchAllRows(() => db.from('contact').select('*').order('id')),
      ]);
      ...
      return {
        ...
        tasks: Object.fromEntries(
          all.filter((r) => r.kind === 'task' && !r.parent_id).map((r) => [r.id, taskRowToTask(r)])
        ),
         ...
         *
         * ⚠️ REPLIES ARE PARTITIONED OUT HERE, and that is deliberate. A reply
         * is an `activity` row like any other, so anything iterating this map
         * -- the feed page, a matter's Activity tab -- would render it as a
         * post of its own. Splitting at load means none of that code has to
         * know replies exist, and none of it can leak one by forgetting a
         * filter. Fail-safe, rather than fail-if-everyone-remembers.
         */
        activity: Object.fromEntries(
          all.filter((r) => !r.parent_id).map((r) => [r.id, activityRowToEntry(r)])
        ),
        replies: repliesByParent(all),
        sections,
        // Everyone, for @mentions and the backup export.
        team: Object.fromEntries(
          (profiles.data || []).map((p) => [p.display_name, p.email])
        ),
        /*
         * Who work can be ASSIGNED to, which is a smaller list: no shared
         * mailboxes, nobody who has left. `is_role_account` and `is_active`
         * have been on the profile table since day one and nothing read them.
         */
        people: assignablePeople(profiles.data || []),
    ...
    async createTask(input = {}) {
      const { data, error } = await db
        ...
        .insert({
          ...
          /*
           * Was missing entirely, so every task made from the Tasks tab landed
           * with a null author and read as "Unknown" -- while the same task
           * made inside a project showed the right name, because addActivity
           * below has always written this.
           *
           * A snapshot, not a join: it survives the profile being removed. The
           * `author_id` column exists too and neither path sets it, which is a
           * separate gap and not this one.
           */
          author_label: input.author || null,
        })
        .select('id')
        .single();
    ...
    /*
     * ONE FIELD, not the whole section. See supabase/017.
     *
     * This used to select every field, merge one key in JavaScript, and
     * write them all back -- so it overwrote anything another person had
     * saved in between, and reverted the section to whatever this browser
     * last loaded. Three of the firm's four reported symptoms were this.
     *
     * The merge now happens inside Postgres, against the current row,
     * under the lock of the statement doing the writing.
     */
    async setSectionField(matterId, sectionKey, fieldKey, value) {
      const { error } = await db.rpc('set_section_field', {
        p_matter_id: matterId,
        p_section_key: sectionKey,
        p_field_key: fieldKey,
        // undefined is not JSON; null is, and it is what "cleared" means.
    ...
    /*
     * Same read-modify-write race as setSectionField, plus a worse one.
     *
     * ⚠️ AN UPDATE THAT MATCHES NO ROW IS NOT AN ERROR. Postgres is asked
     * to change zero rows and does exactly that. The old version returned
     * ok() for it, so an edit typed into a row whose insert had not landed
     * yet was reported as saved and stored nowhere. That is the firm's
     * "we're entering lines, but then they disappear".
     *
     * The function returns the id it wrote. Nothing back means nothing was
     * written, and that is now an error the caller can act on.
     */
    async updateSectionRow(matterId, sectionKey, rowId, patch) {
      const { data, error } = await db.rpc('update_section_row', {
        p_row_id: rowId,
        p_patch: patch || {},
      });
      if (error) return fail(error);
      if (!data) {
        return {
          ok: false,
          error: 'That row is not saved yet — your change was not stored. Refresh the page and try again.',
        };
      }
      return ok();
    },
    ...
    async addActivity(input = {}) {
      const { data, error } = await db
        ...
        .insert({
          ...
          body: input.body || '',
          /*
           * null, not 'You'. 'You' is not a name -- it reads correctly only to
           * the person who wrote it and is nonsense to everyone else looking at
           * the same feed. Same rule as createTask above: the read path
           * supplies a display fallback, the column stores the truth.
           */
          author_label: input.author || null,
     ...
     * Reply to an entry.
     *
     * Carries the parent's matter so a thread on a case stays on that case --
     * without it a reply would be a firm-wide note that happened to hang off
     * something, and deleting the case would leave it orphaned rather than
     * cascading with everything else.
     */
    async addReply(parentId, input = {}) {
      const body = String(input.body || '').trim();
      if (!parentId) return { ok: false, error: 'No entry to reply to.' };
      if (!body) return { ok: false, error: 'A reply needs something in it.' };

      const { data: parent, error: readErr } = await db
        .from('activity').select('id, matter_id').eq('id', parentId).maybeSingle();
      if (readErr) return fail(readErr);
      if (!parent) return { ok: false, error: 'That entry no longer exists.' };
      ...
      const { data, error } = await db
        ...
        .insert({
          ...
          // A reply is a note. Never a task -- a thread that could silently
          // create tasks would put work on a list nobody agreed to.
          kind: 'note',
          body,
          author_label: input.author || null,
          mentions: input.mentions || [],
        ...
        .single();
      if (error) return fail(error);
      return { ok: true, id: data.id, reply: activityRowToEntry(data) };
    },
