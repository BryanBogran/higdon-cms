...
/** Remove a reply by id alone -- a DELETE gives us nothing but the key. */
function removeReplyById(replies, replyId) {
  for (const [parentId, thread] of Object.entries(replies)) {
    if (!(thread || []).some((r) => r.id === replyId)) continue;
    return { ...replies, [parentId]: thread.filter((r) => r.id !== replyId) };
  }
  return null;
}
  ...
  switch (table) {
    ...
    case 'activity': {
      if (type === 'DELETE') {
        ...
        // A deleted REPLY carries only its primary key, so the thread it
        // belonged to has to be found the same way a deleted section row is.
        const nextReplies = removeReplyById(replies, id);
        if (!nextActivity && !nextTasks && !nextReplies) return null;
        return {
          ...(nextActivity ? { activity: nextActivity } : {}),
          ...(nextTasks ? { tasks: nextTasks } : {}),
          ...(nextReplies ? { replies: nextReplies } : {}),
        };
      }
      if (!row?.id) return null;

      /*
       * ⚠️ A REPLY IS AN `activity` ROW. Without this branch a reply arriving
       * live would be dropped into the feed as a post of its own -- the exact
       * leak loadAll partitions to prevent, reintroduced through the back
       * door. Same rule, both entry points.
       */
      if (row.parent_id) {
        const entry = activityRowToEntry(row);
        const thread = replies[row.parent_id] || [];
        const i = thread.findIndex((r) => r.id === row.id);
        const next = i === -1 ? [...thread, entry] : thread.map((r, k) => (k === i ? entry : r));
        next.sort((a, b) => String(a.createdAt).localeCompare(String(b.createdAt)));
        return { replies: { ...replies, [row.parent_id]: next } };
      }
