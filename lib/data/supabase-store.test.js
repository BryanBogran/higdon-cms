...
/* ------------------------------------------------------------------ *
 * An import must generate the deadlines the dates imply
 * ------------------------------------------------------------------ */

test('importing an SOL generates its deadline task, not just the date', async () => {
  /*
   * THE BUG THIS EXISTS FOR. updateMatterField regenerated the chain whenever
   * sol/trialDate/dco changed, but importMatters wrote those columns straight
   * to the table and never did. So the firm's 268 statutes of limitations would
   * have imported as 268 stored dates and ZERO deadline tasks -- a case showing
   * a populated SOL beside an empty Deadlines tab, which reads as "the deadline
   * engine is broken" rather than "the import skipped a step".
   */
  const db = fakeSupabase({ matter: [], matter_checklist_item: [], activity: [] });
  const store = createSupabaseStore(db);
  ...
  const auto = db._rows('activity').filter((a) => a.source === 'auto');
  const sol = auto.find((a) => a.rule_key === 'sol');
  assert.ok(sol, 'importing an SOL should produce an SOL deadline task');
  assert.equal(sol.due_date, '2027-01-20');
  assert.equal(sol.kind, 'task');
});

test('an import with no chain-driving dates generates no deadlines', async () => {
  // The other half of the rule: a referral-only or settlement-only file must
  // not walk every case regenerating chains for dates it never carried.
  const db = fakeSupabase({ matter: [], matter_checklist_item: [], activity: [] });
  const store = createSupabaseStore(db);

  await store.importMatters([
    { rowIndex: 0, action: 'create', values: { clientName: 'Nguyen, Thanh', caseNumber: '26-003', referral: 'Google' }, checklist: {} },
  ]);

  assert.equal(db._rows('activity').filter((a) => a.source === 'auto').length, 0);
});

test('re-importing the same SOL does not add a second deadline', async () => {
  // The import is explicitly re-runnable, so the chain write has to stay
  // idempotent across runs, not just within one.
  const db = fakeSupabase({ matter: [], matter_checklist_item: [], activity: [] });
  const store = createSupabaseStore(db);

  const entry = { rowIndex: 0, action: 'create', values: { clientName: 'Okafor, Ada', caseNumber: '26-002', sol: '2027-01-20' }, checklist: {} };
  await store.importMatters([entry]);
  const id = db._rows('matter')[0].id;
  await store.importMatters([{ ...entry, rowIndex: 0, action: 'update', matterId: id }]);

  const sols = db._rows('activity').filter((a) => a.rule_key === 'sol');
  assert.equal(sols.length, 1, `expected one SOL deadline, found ${sols.length}`);
});

/* ------------------------------------------------------------------ *
 * Linking a contact card has to survive a refresh
 * ------------------------------------------------------------------ */

test('rowsToMatters reads client_contact_id back', () => {
  /*
   * THE BUG. linkClientContact wrote the column and rowsToMatters did not read
   * it, so a linked contact card held until the page was refreshed and then
   * silently came undone. Staff relinked the same card on every visit and it
   * looked like the save had failed -- the save was fine.
   *
   * The two lines above it in the source carry the same warning about
   * drive_folder_id, which was this exact omission the first time.
   */
  const out = rowsToMatters([matterRow({ client_contact_id: 'c9' })], []);
  assert.equal(out.m1.clientContactId, 'c9');
});

test('an unlinked case reads as empty, not undefined', () => {
  // ClientCard tests `matter?.clientContactId ? ... : null`, so '' and
  // undefined behave alike there -- but ContactEditor filters on it, and a
  // missing key would drop the "cases using this contact" list.
  const out = rowsToMatters([matterRow({ client_contact_id: null })], []);
  assert.equal(out.m1.clientContactId, '');
});

test('linking a contact then reloading keeps the link', async () => {
  /*
   * The round trip, which is where the bug actually lived: each half worked
   * and the seam between them did not. Testing only the write would have
   * passed.
   */
  const db = fakeSupabase({
    matter: [matterRow()],
    matter_checklist_item: [],
    contact: [{ id: 'c9', first_name: 'Greicy', last_name: 'Alvarado' }],
  });
  const store = createSupabaseStore(db);

  const res = await store.linkClientContact('m1', 'c9', 'Alvarado, Greicy');
  assert.equal(res.ok, true);

  const data = await store.loadAll();
  assert.equal(
    data.matters.m1.clientContactId,
    'c9',
    'the link must survive a reload — this is the refresh that used to undo it',
  );
  assert.equal(data.matters.m1.values.clientName, 'Alvarado, Greicy');
});

test('unlinking survives a reload too', async () => {
  const db = fakeSupabase({
    matter: [matterRow({ client_contact_id: 'c9' })],
    matter_checklist_item: [],
  });
  const store = createSupabaseStore(db);

  await store.linkClientContact('m1', '', '');
  const data = await store.loadAll();
  assert.equal(data.matters.m1.clientContactId, '', 'an unlink must not come back');
});

/* ------------------------------------------------------------------ *
 * A task remembers who made it
 * ------------------------------------------------------------------ */

test('createTask records the author', async () => {
  /*
   * THE BUG. createTask inserted no author_label, so every task made from the
   * Tasks tab read as "Unknown" in the feed -- while the same task made inside
   * a project showed the right name, because addActivity had always written it.
   *
   * Silent, too: the task saved correctly and simply had no author, which looks
   * like the feed being wrong rather than the insert being incomplete.
   */
  const db = fakeSupabase({ activity: [] });
  const store = createSupabaseStore(db);

  const res = await store.createTask({ title: 'Order records', author: 'Bryan Bogran' });
  assert.equal(res.ok, true);

  const row = db._rows('activity').find((a) => a.id === res.id);
  assert.equal(row.author_label, 'Bryan Bogran');
  assert.equal(row.kind, 'task');
});

test('createTask and addActivity agree about the author column', async () => {
  // The two paths write the same table and differed on one field, which is
  // exactly how this went unnoticed. They must not drift again.
  const db = fakeSupabase({ activity: [] });
  const store = createSupabaseStore(db);

  const t = await store.createTask({ title: 'x', author: 'Same Person' });
  const a = await store.addActivity({ body: 'y', author: 'Same Person' });

  const rows = db._rows('activity');
  assert.equal(rows.find((r) => r.id === t.id).author_label, 'Same Person');
  assert.equal(rows.find((r) => r.id === a.id).author_label, 'Same Person');
});

test('a task with no author supplied stores null, not the string "Unknown"', async () => {
  // 'Unknown' is a UI fallback for a missing value, not a value. Writing the
  // word would make a real author called nothing indistinguishable from an
  // author we failed to record.
  const db = fakeSupabase({ activity: [] });
  const store = createSupabaseStore(db);
  const res = await store.createTask({ title: 'x' });
  assert.equal(db._rows('activity').find((a) => a.id === res.id).author_label, null);
});

test("neither write path stores a placeholder in place of a name", async () => {
  /*
   * 'Unknown' and 'You' are DISPLAY fallbacks. Stored, they make "we did not
   * record who did this" indistinguishable from a real value -- and that is
   * what needed supabase/016_backfill_task_authors.sql to undo.
   *
   * The read path supplies 'Unknown' (see rowToActivity); the column holds
   * null.
   */
  const db = fakeSupabase({ activity: [] });
  const store = createSupabaseStore(db);
  ...
  const a = await store.addActivity({ body: 'y' });
  const e = await store.createTask({ title: 'z', author: '' });

  const rows = db._rows('activity');
  for (const id of [t.id, a.id, e.id]) {
    const stored = rows.find((r) => r.id === id).author_label;
    assert.equal(stored, null, `expected null, got ${JSON.stringify(stored)}`);
  }
});

/* ------------------------------------------------------------------ *
 * The firm's 2026-09-18 report: entries disappearing, and one person's
 * work vanishing off another person's screen.
 * ------------------------------------------------------------------ */

test('two people saving different fields of one section both survive', async () => {
  /*
   * THE BUG THE FIRM REPORTED. setSectionField used to select every field,
   * merge one key in the browser, and write the whole object back -- so the
   * second save reverted the section to whatever that browser had loaded.
   *
   * "I've seen meds on Ericka's screen, but I couldn't see them on mine.
   *  Then when we went back to look at it on Ericka's screen, it wasn't
   *  there anymore."
   */
  const db = fakeSupabase({ matter_section_data: [] });
  const store = createSupabaseStore(db);

  // CONCURRENTLY -- both in flight at once, which is the whole point. Run
  // one after the other and the old read-merge-write passes too, because
  // the second read sees the first write. The bug only exists when the two
  // overlap, so the test has to overlap them.
  await Promise.all([
    store.setSectionField('m1', 'medicals', 'provider', 'Dr Ruiz'),
    store.setSectionField('m1', 'medicals', 'treatmentstatus', 'Ongoing'),
  ]);

  const [row] = db._rows('matter_section_data');
  assert.deepEqual(row.fields, { provider: 'Dr Ruiz', treatmentstatus: 'Ongoing' });
});

test('a browser that never saw the other edit cannot revert it', async () => {
  // The stale-tab case: the app loads once and never refreshes, so a tab
  // left open all morning holds a copy from 9am. Its next save must not
  // take the section back there.
  const db = fakeSupabase({
    matter_section_data: [{ matter_id: 'm1', section_key: 'medicals', fields: { a: 'from 9am' } }],
  });
  const store = createSupabaseStore(db);

  // Somebody else writes while this browser is mid-save. Overlapping, so
  // the stale tab's request was built before the other write landed.
  await Promise.all([
    store.setSectionField('m1', 'medicals', 'b', 'added at noon'),
    store.setSectionField('m1', 'medicals', 'c', 'from the stale tab'),
  ]);

  const [row] = db._rows('matter_section_data');
  assert.equal(row.fields.b, 'added at noon', "the other person's edit was erased");
  assert.deepEqual(row.fields, { a: 'from 9am', b: 'added at noon', c: 'from the stale tab' });
});

test('the browser sends one field, never a whole section', async () => {
  // The guarantee behind the two tests above: if a whole `fields` object is
  // ever sent again, it can overwrite what this client never saw.
  const db = fakeSupabase({ matter_section_data: [] });
  const store = createSupabaseStore(db);

  await store.setSectionField('m1', 'intake', 'k', 'v');

  const call = db._calls.find((c) => c.op === 'rpc:set_section_field');
  assert.ok(call, 'setSectionField must go through the atomic function');
  assert.equal(call.args.p_field_key, 'k');
  assert.ok(!('fields' in call.args), 'a whole section object was sent');
  assert.ok(!db._calls.some((c) => c.table === 'matter_section_data' && c.op === 'upsert'));
});

test('editing a row that was never saved reports failure, not success', async () => {
  /*
   * "We're entering lines/documents, but then they disappear."
   *
   * A new row is on screen with a browser-made id before the insert lands.
   * An edit during that window targets an id that does not exist. Postgres
   * updates zero rows and reports NO ERROR -- so the old code returned
   * ok() and the value was stored nowhere.
   */
  const db = fakeSupabase({ matter_section_row: [] });
  const store = createSupabaseStore(db);

  const result = await store.updateSectionRow('m1', 'expenses', 'a-browser-made-id', { amount: '250' });

  assert.equal(result.ok, false, 'a write that stored nothing must not report success');
  assert.match(result.error, /not saved yet/i);
});

test('two people editing different cells of one row both survive', async () => {
  const db = fakeSupabase({
    matter_section_row: [{ id: 'r1', matter_id: 'm1', section_key: 'expenses', data: { type: 'Parking' } }],
  });
  const store = createSupabaseStore(db);

  await Promise.all([
    store.updateSectionRow('m1', 'expenses', 'r1', { amount: '40' }),
    store.updateSectionRow('m1', 'expenses', 'r1', { notes: 'court garage' }),
  ]);

  const [row] = db._rows('matter_section_row');
  assert.deepEqual(row.data, { type: 'Parking', amount: '40', notes: 'court garage' });
});

test('a patch cannot rewrite the row id it is stored under', async () => {
  // The row's identity lives in the column, not the payload. A patch
  // carrying one would let a client point a row at another record.
  const db = fakeSupabase({
    matter_section_row: [{ id: 'r1', matter_id: 'm1', section_key: 'expenses', data: { type: 'Parking' } }],
  });
  const store = createSupabaseStore(db);

  await store.updateSectionRow('m1', 'expenses', 'r1', { id: 'r2', amount: '40' });

  const [row] = db._rows('matter_section_row');
  assert.equal(row.id, 'r1');
  assert.ok(!('id' in row.data), 'an id leaked into the row payload');
});

/* ------------------------------------------------------------------ *
 * The 1000-row cap — 45% of the firm's section data was never fetched
 * ------------------------------------------------------------------ */

function manySectionRows(n, { matterId = 'm1', sectionKey = 'meds' } = {}) {
  const rows = [];
  for (let i = 0; i < n; i++) {
    rows.push({
...
test('loadAll returns every section row, not the first 1000', async () => {
  /*
   * THE BUG, EXACTLY AS IT HAPPENED.
   *
   * Supabase caps a response at max-rows (1000 on this project) and reports
   * NOTHING when it truncates -- no error, no flag. The firm had 1,995
   * section rows, so 995 of them had never been fetched since the day they
   * were written, and a case whose rows fell past the cut looked like it had
   * lost its medical records.
   */
  const db = fakeSupabase(
    { matter: [{ id: 'm1', client_name: 'Hollister' }], matter_section_row: manySectionRows(1995) },
    { maxRows: 1000 }
  );
  const data = await createSupabaseStore(db).loadAll();
  assert.equal(data.sections.m1.meds.rows.length, 1995, 'rows were silently dropped');
});

test('paging does not duplicate or skip a row', async () => {
  /*
   * ⚠️ THE SUBTLER BUG PAGING CAN INTRODUCE. Page boundaries only line up if
   * the database returns rows in the SAME sequence every time. Ordering by
   * `ordinal` alone does not guarantee that -- rows sharing an ordinal may
   * come back in any order, so page 2 can repeat one row from page 1 and
   * skip another. Every query ends its ordering on a unique column.
   */
  const rows = manySectionRows(1500);
  for (const r of rows) r.ordinal = 0; // every row ties on the sort key
  const db = fakeSupabase(
    { matter: [{ id: 'm1' }], matter_section_row: rows },
    { maxRows: 1000 }
  );
  const data = await createSupabaseStore(db).loadAll();

  const ids = data.sections.m1.meds.rows.map((r) => r.id);
  assert.equal(ids.length, 1500, 'rows lost or duplicated across pages');
  assert.equal(new Set(ids).size, 1500, 'the same row came back on two pages');
});

test('the feed is paged too, so old entries do not fall off', async () => {
  // activity is ordered newest-first, so truncation silently ate the OLDEST
  // history -- the entries nobody looks at until they need them.
  const activity = [];
  ...
  const db = fakeSupabase({ matter: [{ id: 'm1' }], activity }, { maxRows: 1000 });
  const data = await createSupabaseStore(db).loadAll();
  assert.equal(Object.keys(data.activity).length, 1400);
});

test('checklist items are paged, and they had no ordering at all', async () => {
  // matter_checklist_item was fetched with no order whatsoever, so WHICH
  // items vanished past the cap was arbitrary and could differ between loads.
  const items = [];
  for (let i = 0; i < 1100; i++) {
    items.push({ matter_id: `m${i}`, field_key: 'suitFiled', done: true, occurred_on: '2026-03-01' });
  }
  const matters = items.map((it) => ({ id: it.matter_id }));
  const db = fakeSupabase({ matter: matters, matter_checklist_item: items }, { maxRows: 1000 });
  const data = await createSupabaseStore(db).loadAll();

  const ticked = Object.values(data.matters).filter((m) => m.values.suitFiled?.done).length;
  assert.equal(ticked, 1100, 'checklist state was lost past the cap');
});

test('a page that comes back short is the last page', async () => {
  // The only reliable end signal. Asking for a count costs another round trip
  // and can disagree with the rows under concurrent writes.
  const db = fakeSupabase(
    { matter: [{ id: 'm1' }], matter_section_row: manySectionRows(1000) },
    { maxRows: 1000 }
  );
  const data = await createSupabaseStore(db).loadAll();
  // Exactly one full page: the loop must ask once more and stop on the empty
  // second page rather than returning 1000 and assuming that is all.
  assert.equal(data.sections.m1.meds.rows.length, 1000);
});
...
test('replies are kept out of the feed and out of the task list', async () => {
  /*
   * A reply is an `activity` row, and the feed page and a matter's Activity
   * tab both iterate every entry. Partitioning at LOAD means none of that
   * code has to know replies exist and none of it can leak one by forgetting
   * a filter -- fail-safe rather than fail-if-everyone-remembers.
   */
  const db = fakeSupabase({
    matter: [{ id: 'm1' }],
    activity: [
      { id: 'a1', matter_id: 'm1', kind: 'task', body: 'Order records', source: 'ui' },
      { id: 'r1', matter_id: 'm1', kind: 'note', body: 'Provider has not sent them', parent_id: 'a1', source: 'ui', created_at: '2026-09-18T09:00:00Z' },
      { id: 'r2', matter_id: 'm1', kind: 'note', body: 'Chased again today', parent_id: 'a1', source: 'ui', created_at: '2026-09-18T11:00:00Z' },
    ],
  });
  const data = await createSupabaseStore(db).loadAll();

  assert.deepEqual(Object.keys(data.activity), ['a1'], 'a reply reached the feed');
  assert.deepEqual(Object.keys(data.tasks), ['a1']);
  assert.deepEqual(data.replies.a1.map((r) => r.body), [
    'Provider has not sent them',
    'Chased again today',
  ], 'a thread reads oldest first');
});

test('a reply inherits the case it is answering', async () => {
  // Without this a thread on a case would be a firm-wide note that happened to
  // hang off something, and deleting the case would orphan it rather than
  // cascading it away with everything else.
  const db = fakeSupabase({
    activity: [{ id: 'a1', matter_id: 'm-hollister', kind: 'task', body: 'x', source: 'ui' }],
  });
  const store = createSupabaseStore(db);
  const result = await store.addReply('a1', { body: 'Waiting on the provider', author: 'rose' });

  assert.equal(result.ok, true, result.error);
  const row = db._rows('activity').find((r) => r.id === result.id);
  assert.equal(row.matter_id, 'm-hollister');
  assert.equal(row.parent_id, 'a1');
  assert.equal(row.kind, 'note', 'a reply must never be a task');
});

test('an empty reply is refused rather than stored blank', async () => {
  const db = fakeSupabase({ activity: [{ id: 'a1', matter_id: 'm1', kind: 'note' }] });
  const store = createSupabaseStore(db);
  for (const body of ['', '   ', null, undefined]) {
    const r = await store.addReply('a1', { body });
    assert.equal(r.ok, false, JSON.stringify(body));
  }
});

test('replying to something that no longer exists says so', async () => {
  const db = fakeSupabase({ activity: [] });
  const result = await createSupabaseStore(db).addReply('gone', { body: 'hello' });
