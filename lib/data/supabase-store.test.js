...
test('deleting goes through the function, not a bare delete', async () => {
  /*
   * A plain .delete() from the client would skip three things it cannot do
   * itself: the legal-hold refusal, removing the audit history (which has no
   * foreign key), and writing the tombstone that replaces it.
   */
  const db = fakeSupabase({ matter: [matterRow({ id: 'm1' })] }, {
    rpc: { delete_matter: { ok: true, case_number: '26-001', audit_rows_removed: 12 } },
  });
  const store = createSupabaseStore(db);

  const r = await store.deleteMatter('m1', { reason: 'Duplicate of 26-041' });
  assert.equal(r.ok, true);
  assert.equal(r.audit_rows_removed, 12);
