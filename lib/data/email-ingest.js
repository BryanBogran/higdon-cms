...
function storagePath(matterId, messageKey, key) {
  return `${matterId}/email/${messageKey}/${key}`;
}
...
export async function uploadEmailFiles(db, { matterId, parsed, bytes }) {
  ...
  const originalPath = storagePath(matterId, key, storageKey(original.name));
  ...
  // Indexed, never by name alone: two attachments called image.png would
  // otherwise share a key and the second upsert would erase the first.
  for (const [i, a] of parsed.attachments.entries()) {
    const path = storagePath(matterId, key, attachmentKey(i, a.filename));
