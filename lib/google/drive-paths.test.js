...
import { escapeQ, parentsClause, searchQuery, childrenQuery, indexFolders, isWithinTree, breadcrumb, matterForFile, sortChildren, FOLDER_MIME, MAX_SCOPED_FOLDERS, sortDriveFiles, FILE_SORTS } from './drive-paths.js';
...
const listing = [
  { id: 'f1', name: 'Pleadings', isFolder: true },
  { id: 'a', name: 'Authorization.pdf', sizeBytes: 1338, modifiedTime: '2025-05-21T00:00:00Z' },
  { id: 'b', name: 'attachments.pdf', sizeBytes: 329, modifiedTime: '2026-09-16T00:00:00Z' },
  { id: 'c', name: 'Notes', sizeBytes: null, modifiedTime: '2024-12-03T00:00:00Z' }, // a Google Doc
  { id: 'f2', name: 'Discovery', isFolder: true },
];

test('folders lead, in every sort and both directions', () => {
  // A folder sorted into the middle of a hundred PDFs by date is a folder
  // nobody finds.
  for (const key of FILE_SORTS) {
    for (const ascending of [true, false]) {
      const out = sortDriveFiles(listing, { key, ascending });
      assert.ok(out[0].isFolder && out[1].isFolder, `${key}/${ascending} buried a folder`);
    }
  }
});

test('newest first is a real option, and it reverses', () => {
  const desc = sortDriveFiles(listing, { key: 'modified', ascending: false })
    .filter((r) => !r.isFolder).map((r) => r.id);
  assert.deepEqual(desc, ['b', 'a', 'c']);

  const asc = sortDriveFiles(listing, { key: 'modified', ascending: true })
    .filter((r) => !r.isFolder).map((r) => r.id);
  assert.deepEqual(asc, ['c', 'a', 'b']);
});

test('a Google Doc has no size, and absent is not zero', () => {
  /*
   * THE TRAP. Google Docs carry no byte size at all. Treating that as 0 files
   * every Doc under "smallest" and makes the ascending view look like a list
   * of empty files. A row with no value sorts to the END either way -- "I
   * don't know" is not a position on the scale.
   */
  const asc = sortDriveFiles(listing, { key: 'size', ascending: true })
    .filter((r) => !r.isFolder).map((r) => r.id);
  assert.deepEqual(asc, ['b', 'a', 'c'], 'the Doc should sink, not lead');

  const desc = sortDriveFiles(listing, { key: 'size', ascending: false })
    .filter((r) => !r.isFolder).map((r) => r.id);
  assert.deepEqual(desc, ['a', 'b', 'c'], 'the Doc should sink in this direction too');
});

test('names sort the way a reader expects, not by code point', () => {
  // "attachments.pdf" belongs beside "Authorization.pdf", not after every
  // capital letter in the alphabet.
  const names = sortDriveFiles(listing, { key: 'name', ascending: true })
    .filter((r) => !r.isFolder).map((r) => r.name);
  assert.deepEqual(names, ['attachments.pdf', 'Authorization.pdf', 'Notes']);
});

test('sorting does not mutate the caller\'s array', () => {
  const original = [...listing];
  sortDriveFiles(listing, { key: 'size', ascending: false });
  assert.deepEqual(listing, original);
});

test('an empty or missing listing is not a crash', () => {
  assert.deepEqual(sortDriveFiles([], { key: 'size' }), []);
  assert.deepEqual(sortDriveFiles(), []);
  assert.deepEqual(sortDriveFiles(null, { key: 'name' }), []);
});
