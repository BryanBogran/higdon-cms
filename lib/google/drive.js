...
/**
 * Find the named subfolder of a case, creating it if it is missing.
 *
 * Lets a field say "put this in Medical Records" without knowing any folder
 * ids. Older cases, and folders adopted rather than created by this app, will
 * not have our standard subfolders — so it makes one rather than dumping a
 * medical record loose in the case root.
 *
 * Matching is case-insensitive and trims, because "Medical records" and
 * "Medical Records " are the same folder to a human and creating a second one
 * would split the case's records across both.
 */
export async function resolveSubfolder(rootId, name, { token, create = true } = {}) {
  const auth = token ? { ok: true, token } : await getAccessToken();
  if (!auth.ok) return auth;
  if (!name) return { ok: true, folderId: rootId, created: false };

  const listed = await listChildFolders(rootId, { token: auth.token });
  if (!listed.ok) return listed;

  const wanted = String(name).trim().toLowerCase();
  const found = listed.folders.find((f) => String(f.name || '').trim().toLowerCase() === wanted);
  if (found) return { ok: true, folderId: found.id, folderName: found.name, created: false };

  if (!create) return { ok: true, folderId: rootId, created: false };

  const made = await createFolder({ parentId: rootId, name, token: auth.token });
  if (!made.ok) return made;
  return { ok: true, folderId: made.folder.id, folderName: made.folder.name, created: true };
}
 ...
 */
/**
 * Start a resumable upload the browser will finish.
 *
 * `origin` matters more than it looks: Google binds CORS to the session at
 * creation, so without it the final response to the browser's PUT carries no
 * Access-Control-Allow-Origin and the browser throws away a successful upload.
 * See the note at the call site in app/api/drive/upload-url/route.js.
 */
export async function createUploadSession({ parentId, name, mimeType, sizeBytes, token, origin }) {
  const auth = token ? { ok: true, token } : await getAccessToken();
  if (!auth.ok) return auth;
