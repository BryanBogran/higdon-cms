...
export const FILE_SORTS = ['name', 'modified', 'size'];

/**
 * Sort a Drive listing.
 *
 * ── Folders first, always ────────────────────────────────────────────────
 *
 * Whatever the sort, folders stay at the top. They are navigation rather than
 * content, and a folder sorted into the middle of a hundred PDFs by date is a
 * folder nobody finds.
 *
 * ── Absent is not zero, and not empty ────────────────────────────────────
 *
 * ⚠️ A Google Doc carries NO byte size at all -- `sizeBytes` is absent, not 0.
 * Treating it as zero would file every Doc under "smallest" and make the
 * ascending view look like a list of empty files. Rows with no value for the
 * chosen key sort to the END in both directions: "I don't know" is not a
 * position on the scale, so it does not get one.
 *
 * Names compare with localeCompare so that accents and case sort the way a
 * person reading the list expects, rather than by code point.
 */
export function sortDriveFiles(rows = [], { key = 'name', ascending = true } = {}) {
  const dir = ascending ? 1 : -1;

  const valueOf = (r) => {
    if (key === 'size') return r?.sizeBytes ?? null;
    if (key === 'modified') return r?.modifiedTime || null;
    return String(r?.name ?? '');
  };

  return [...(rows || [])].sort((a, b) => {
    // Folders are navigation. They lead, in every sort and both directions.
    ...
    // Missing values sink, regardless of direction -- see the note above.
