...
/**
 * The 13-item litigation checklist.
 *
 * Exactly what the requirements note asks for: "each thing should have a space
 * to say yes or no when completed, and then have a link upload to upload our
 * Google Drive link with the specific document."
 *
 * Marking an item done stamps today's date in the firm's timezone -- that date is
 * the trigger five of the eight chain rules read, so an item marked done with no
 * date produces no deadline at all. The date input is therefore always visible
 * and always editable, not hidden behind a second click.
 */
...
const CHECKLIST = FIELDS.filter((f) => f.type === 'yesnoDoc');
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
      <div className="px-5 py-3 border-b border-slate-100">
        <h2 className="font-semibold text-slate-900">Litigation Checklist</h2>
