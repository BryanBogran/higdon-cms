...
/*
 * PORTED VERBATIM from prototype/higdon-cms.jsx. Three edits only:
 *   1. this header ("use client" -- it uses hooks, browser storage and Papa.parse
 *      on a File object, so it cannot render on the server)
 *   2. the module-scope document.createElement("style") block deleted; the
 *      .input class now lives in app/globals.css
 *   3. installStorageShim() called at module scope, mapping the Claude artifact
 *      `window.storage` API onto localStorage
 *
 * Nothing else is touched. Phase 1's only deliverable is "the build works", so
 * the known bugs stay in place deliberately -- prototype/ is the diff baseline.
 * Logic extraction is Phase 2; the storage call sites are Phase 4.
 */
